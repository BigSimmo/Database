...
// DSN, @sentry/nextjs is never imported, so tests and DSN-less deployments never touch
...
// (see sentry.server.config.ts beforeSend), but callers are the first line of defense.
...
async function loadSentry(): Promise<SentryLike | null> {
  ...
  sentryModule ??= import("@sentry/nextjs").then(
    (mod) => mod as SentryLike,
    // A missing/broken SDK must degrade to a no-op, never break a request.
    () => null,
  );
  return sentryModule;
}
