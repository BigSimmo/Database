...
/** Treat blank/whitespace as unset so offline scrub can pin `SENTRY_DSN=""`. */
const blankOptionalUrl = (value: unknown) => (typeof value === "string" && value.trim() === "" ? undefined : value);
...
const envSchema = z.object({
  ...
  // Keep `z.`-prefixed declarations so scripts/check-env-parity.mjs collects names.
  NEXT_PUBLIC_SENTRY_DSN: z.preprocess(blankOptionalUrl, z.string().url().optional()),
