 ...
 */
type FormulationHomeRouteProps = {
  searchParams?: Promise<Record<string, string | string[] | undefined>>;
};

export default async function FormulationHomeRoute({ searchParams }: FormulationHomeRouteProps) {
  ...
  // submitted `?q=…&run=1`, which goes on to /formulation/search rather than
  // arriving at the home with its query dropped.
  const params = searchParams ? await searchParams : {};
  redirect(consolidatedModeHomeTargetForSearchParams("/formulation", params) ?? appModeSelectionHref("formulation"));
}
