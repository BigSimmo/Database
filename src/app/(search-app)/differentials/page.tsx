...
/**
 * `Differential Diagnosis` has no home page of its own any more.
 *
...
export default async function DifferentialsHomeRoute({ searchParams }: DifferentialsHomeRouteProps) {
  ...
  // submitted `?q=…&run=1`, which goes on to /differentials/search rather than
  // arriving at the home with its query dropped.
  const params = searchParams ? await searchParams : {};
  redirect(
    consolidatedModeHomeTargetForSearchParams("/differentials", params) ?? appModeSelectionHref("differentials"),
  );
}
