 ...
 *
 * Requires BOTH `leftAt` and `leavingDestination` — an admission whose `state` is `"departed"` but is
 * missing either is incoherent data, and the conservative answer is silence, never a guessed
 * destination or a guessed instant.
 *
 * **Gated on `leftAt <= now`, never on a bare `state === "departed"` check.** `now` is supplied by the
