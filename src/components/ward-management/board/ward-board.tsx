...
export function WardBoard({ unitId }: { unitId: string }) {
  ...
  const { units: liveUnits, admissions, now, dispatch } = useWardFlow();
                ...
                {selectedTile.kind === "occupied" || selectedTile.kind === "waiting" ? (
                  selectedOccupant !== null ? (
                    <div className={styles.person} data-testid="ward-board-detail-person">
                      ...
                      {/*
                       * RECORDING THAT THIS PERSON HAS LEFT — the first control on this board that
                       * changes anything, and the only one the ward's own screen never had.
                       *
                       * ⚠️ OCCUPIED ONLY, never `waiting`. A waiting tile is a bed this ward has
                       * given away to somebody who has not arrived; that person has never occupied
                       * it, so they cannot leave it. The reducer refuses it anyway — this is the
                       * second of the two, not the only one — but offering a control that will be
                       * refused is how a prototype teaches a clinician to distrust its controls.
                       *
                       * The destination is CHOSEN, never derived. Exactly one of the five
                       * (`transferred-to-another-psychiatric-ward`) does not count as a statewide
                       * release, because that bed is still occupied somewhere in the system — which
                       * is precisely the fact nobody else can infer from a discharge happening.
                       */}
                        ...
                        <div className={styles.leaving} data-testid="ward-board-record-leaving">
                          ...
                          <button
                            ...
                            onClick={() => {
                              ...
                              // The selection is closed because the person it named has gone. Left
                              // open, the panel would describe a bed this ward no longer fills.
                              closeDetail();
                            }}
                          >
                      ...
                      {/*
                       * AWAY AT AN EMERGENCY DEPARTMENT — the two halves of one fact, on one
                       * control that reads the record rather than a local flag.
                       *
                       * ⚠️ **BOTH HALVES OR NEITHER.** `awayAtEmergencyDepartmentSince` had a
                       * renderer, a seed and no event at either end until 2026-09-01: nothing could
                       * mark somebody away, and — worse — nothing could mark them back, so the
                       * seeded rows counted upward without bound for as long as the demonstration
                       * ran. Shipping only the "gone" button would have been the same defect turned
                       * round: a badge nobody can remove.
                       *
                       * Which button shows is decided by `occupant.awayAtEdHours`, which is derived
                       * from the record itself. Never a `useState` mirroring it — a local flag is
                       * how a control comes to report an act that never happened, and this board's
                       * own history has one of those in it.
                       *
                       * OCCUPIED ONLY, for the same reason the discharge control above is: a
                       * `waiting` tile is a bed given away to somebody who has not arrived, and
                       * they cannot leave a ward they have never reached. The reducer refuses it
                       * too — this is the second of the two, not the only one.
                       *
                       * ⚠️ **NOTHING HERE MOVES A BED.** The panel says so in words directly below,
                       * because "away" on a bed board otherwise reads as "so the bed is free" and
                       * it is not.
                       */}
                      {selectedTile.kind === "occupied" && (
                        <div className={styles.leaving} data-testid="ward-board-away-at-ed">
                          {selectedOccupant.awayAtEdHours === null ? (
                            <button
                              type="button"
                              className={styles.awayButton}
                          ...
                          <p className={styles.personLine}>
                            The bed stays theirs either way. Recording a trip to an emergency department changes nothing
                            this ward can offer a coordinator.
                          </p>
                        </div>
                      )}
                    </div>
