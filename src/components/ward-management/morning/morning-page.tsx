...
/**
 * Task 2 (Phase 6). The morning bed state: five figures in one vocabulary, network-wide down to
 * hospital down to ward.
 *
 * ⚠️ **THIS PAGE IS LIVE. THE COMMENT THAT USED TO SIT HERE SAID IT WAS FROZEN, AND HAD BEEN WRONG
 * SINCE `WB-DB-11`.** It described a `useState` initialiser closing over a single read of
 * `useWardFlow()`, said "THE FREEZE MUST BE REAL", and named `handover/handover-page.tsx` as
 * "exactly the mechanism" it used. All three are now false: this file contains no `useState` call
 * at all, and the handover page reads live too (owner decision OD-4, 2026-08-30).
 *
 * **It is replaced rather than deleted because the failure is worth naming.** On 2026-09-01 a
 * defect reached the screen for precisely this reason one component over — the ward board carried
 * a comment asserting it read no live state, and the assertion had outlived the code. Two separate
 * agents read THIS file and reached opposite conclusions from it. A comment that recruits another
 * file as its example decays when that other file changes, and nothing local ever fails.
 *
 * What is true: `liveRollup` below is recomputed from `serviceRollup(...)` at the live `now` on
 * every render, exactly like `DischargeBoard`. There is nothing here a coordinator needs held
 * still while chasing a live bed.
 *
 * ⚠️ **`FrozenMorning` and `buildFrozenMorning` are still exported above and have ZERO references
 * anywhere in src, tests or scripts.** They are the machinery of the deleted freeze. They are NOT
 * removed here, because a reachability count is not sufficient grounds in this repository — run
 * `npm run check:dead-code-candidate` first. But nobody should reinstate them by trusting a
 * comment: the freeze was an owner decision, and it was reversed by a later one.
 *
  ...
  return (
    <div className={styles.screen} data-testid="ward-morning-page">
      ...
      <main id="main-content" className={styles.main}>
         ...
         * nothing, and `tests/ward-morning-tour-paused.dom.test.tsx` asserts exactly that rather than
