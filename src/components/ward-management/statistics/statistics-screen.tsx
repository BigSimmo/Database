  ...
  return (
    <div className={styles.screen} data-testid="ward-statistics-screen">
      ...
      <main id="main-content" className={styles.main}>
        ...
        {/* ── Audience 1 ─────────────────────────────────────────────────────────────────── */}
        <section className={styles.section} data-testid="ward-statistics-system">
          <header className={styles.sectionHeader}>
            <h2 className={styles.sectionHeading}>How the system is performing</h2>
          ...
          <article className={styles.figure} data-testid="ward-statistics-bed-readiness">
            <h3 className={styles.figureHeading}>Beds being made ready</h3>
            ...
            <p className={styles.measuredCount} data-testid="ward-statistics-preparing-count">
              ...
              {preparingCount === 1 ? "bed is" : "beds are"} currently marked as being made ready.
            </p>
            {/*
            ...
            <p className={styles.figureNote}>
              A measured count, whatever its value. Nought here means no bed is currently marked as being prepared — it
              does not mean the count is unavailable. Every bed counted here should already be free: preparation only
              ever begins after somebody has left, so this should never be a count of discharges still to come. Should,
              rather than is — the only screen that can raise the flag offers it on released beds alone, but nothing in
              the model enforces that, so a bed nobody has left yet could be flagged and would be counted here. This
              page counts the flag as it finds it rather than quietly filtering to the beds it expects, because a filter
              would drop such a record and hide the very inversion it was meant to prevent.
            </p>
          ...
          <article className={styles.figure} data-testid="ward-statistics-refused-so-far">
            ...
            <p className={styles.figureNote} data-testid="ward-statistics-refused-so-far-why-so-far">
              <strong>
                The heading says &ldquo;so far&rdquo; because that is as far as the record goes, not because the figure
                is being hedged.
              </strong>{" "}
              Nothing on a movement marks the network as exhausted — there is no closure flag, no cap-reached marker and
              no &ldquo;nobody left to ask&rdquo; state anywhere on the record — so exhaustion is not a thing this data
              can express, and the count above is exactly what it says: refusals on record, nothing pending. A refusal
              takes that ward off the list of wards deciding and leaves the movement at the stage referrals are made
              from, so a coordinator can put it to fresh wards the moment a decline lands; a movement counted here now
              can be back with several wards a minute later. And a movement can be live at only{" "}
              <span data-testid="ward-statistics-refused-so-far-cap">{PARALLEL_REFERRAL_CAP}</span> wards at once, so a
              movement counted here has been put to at most that many wards out of the whole network, and the rest have
              never been asked. Nothing on the record measures how many it was actually put to, so that is a ceiling and
              never a typical figure. This is a worklist of who needs a decision today. It is not a count of patients
            ...
            <p className={styles.measuredCount} data-testid="ward-statistics-refused-so-far-escalated">
              ...
              {refused.escalatedCount === 1 ? "movement carries" : "movements carry"} a recorded escalation instead. The
              derivation this page shares with the handover screen classifies an escalation first, so any of those that
              also meet the condition above are absent from the count — which makes the count a floor rather than the
              whole of it. An escalation is somebody&apos;s recorded opinion that the network was exhausted, never a
              derived fact, so it is disclosed here and relied on for nothing.
            </p>
          </article>
          ...
          <article className={styles.figure} data-testid="ward-statistics-declines">
            ...
            <p className={styles.absence} data-testid="ward-statistics-declines-withheld">
              <strong>
                This figure is withheld pending an owner ruling. It is not that no ward declines, and not that declines
          ...
          <article className={styles.figure} data-testid="ward-statistics-declines-by-reason">
            ...
            <p className={styles.figureNote} data-testid="ward-statistics-declines-by-reason-generated">
              <strong>
                Every one of the{" "}
                <span data-testid="ward-statistics-declines-by-reason-vocabulary-size">{declines.vocabularySize}</span>{" "}
              ...
              generate that row at all, with nothing going red. The rows come from the model&apos;s own list of reasons
              rather than from anything written on this page, so a reason added to the model appears here on its own and
              a reason removed cannot linger; the row count and the number above are the same number, and a reader can
              check that by counting.
            </p>
            <p className={styles.figureNote}>
              The order is the model&apos;s own and is not a ranking — nothing here sorts a reason to the top for being
              common. Historical rather than a picture of tonight: a refusal is a thing that happened and still counts
              after the movement it was made against has reached a bed, so movements that have since closed are
              included.
            </p>
          </article>
          ...
          <article className={styles.figure} data-testid="ward-statistics-blocked-discharges-by-reason">
            ...
            <p className={styles.figureNote} data-testid="ward-statistics-blocked-discharges-by-reason-generated">
              ...
              rather than from anything written on this page, so the row count and the number above are the same number,
              and a reader can check that by counting.
            </p>
          </article>
        </section>

        {/* ── Audience 2 ─────────────────────────────────────────────────────────────────── */}
        <section className={styles.section} data-testid="ward-statistics-patients">
          <header className={styles.sectionHeader}>
            <h2 className={styles.sectionHeading}>What is happening to patients</h2>
          ...
          <article className={styles.figure} data-testid="ward-statistics-pull-to-arrival">
              ...
              <>
                ...
                arrivals.shortestMinutes === arrivals.longestMinutes ? (
                  <p className={styles.figureNote} data-testid="ward-statistics-arrival-constant-gap">
                    <strong>
                      Every measured gap here is the same length, so this average is describing no variation at all: it
                      is one value repeated, not a measurement of how long beds take to fill.
                    </strong>{" "}
                    ...
                    service does not hand back the same gap for everybody — but that is an explanation this shape points
                    at rather than a finding the page has established. Either way nothing here can widen the figure: if
                    these gaps are meant to differ, they have to be written that way on the admissions side, which is a
                    change to the fixture rather than to this page, and the moment they do differ the two ends part and
                    this paragraph disappears on its own.
                  </p>
                ) : null}
              </>
            )}
            ...
            <p className={styles.measuredCount} data-testid="ward-statistics-arrival-incoherent">
              ...
              {arrivals.incoherentCount === 1 ? "admission records an arrival" : "admissions record an arrival"} EARLIER
              than the bed was given away. That cannot be true, so{" "}
              {arrivals.incoherentCount === 1 ? "it is" : "they are"} excluded from every figure above and counted here
              instead — never folded in as a wait of no time at all.
            </p>
          </article>
          ...
          <article className={styles.figure} data-testid="ward-statistics-referral-to-bed">
            ...
            <p className={styles.absence} data-testid="ward-statistics-referral-join-absent">
              ...
              PRODUCED the bed. A referral raised about somebody already in the bed carries a perfectly good instant,
              matches a perfectly good id, and dates the wrong event; all the arithmetic can see is whether the referral
              was raised before the person arrived, which is why that count is reported on its own rather than folded
              into an average. So this page counts what it can establish and stops there. Squaring an incoherent pair
              away with an absolute value or a floor of nought would turn the wrong event into a confident average, on
              the one screen where a plausible figure is never re-checked. Publishing the duration needs the front-door
              request recorded as such, and the admission pointed at it — a change to the data rather than to this page.
            </p>
            ...
            <p className={styles.measuredCount} data-testid="ward-statistics-join-count">
              ...
              {join.joinedCount === 1 ? "pair" : "pairs"} could carry a duration at all — that is, the person arrived no
              earlier than the referral was raised.
            </p>
