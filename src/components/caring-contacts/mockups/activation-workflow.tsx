...
export function ActivationWorkflow({
  ...
  return (
    <div className="space-y-5">
      ...
      {stage === "agreement" ? <AgreementStage onContinue={() => onStageChange("pathway")} /> : null}
      {stage === "pathway" ? (
        <PathwayStage onBack={() => onStageChange("agreement")} onContinue={() => onStageChange("personalisation")} />
      ) : null}
      {stage === "personalisation" ? (
        <PersonalisationStage onBack={() => onStageChange("pathway")} onContinue={() => onStageChange("review")} />
      ) : null}
      {stage === "review" ? (
        <ReviewStage onBack={() => onStageChange("personalisation")} onActivated={onActivated} />
      ) : null}
    </div>
  );
}

function AgreementStage({ onContinue }: { onContinue: () => void }) {
  const patient = syntheticPatients[1];
  const coordinator = syntheticTeamMembers[0];
  return (
    <div
      className="grid min-w-0 gap-5 lg:grid-cols-[minmax(0,1.25fr)_minmax(18rem,0.75fr)]"
      data-testid="caring-contact-screen-patient-agreement"
    >
...
function PathwayStage({ onBack, onContinue }: { onBack: () => void; onContinue: () => void }) {
  ...
  return (
    <div className="space-y-5" data-testid="caring-contact-screen-pathway-selection">
      <div className="grid min-w-0 gap-5 lg:grid-cols-[minmax(17rem,0.78fr)_minmax(0,1.22fr)]">
        ...
        <ProductSection labelledBy="pathway-detail-heading" className="overflow-hidden">
          ...
          <div className="flex justify-end border-t border-[color:var(--border)] px-5 py-3">
            <Button ref={previewTriggerRef} variant="secondary" icon={Eye} onClick={() => setPreviewOpen(true)}>
              Preview pathway
            </Button>
          </div>
        </ProductSection>
...
function PersonalisationStage({ onBack, onContinue }: { onBack: () => void; onContinue: () => void }) {
  return (
    <div className="space-y-5" data-testid="caring-contact-screen-personalisation">
      <div className="grid min-w-0 gap-5 lg:grid-cols-[minmax(0,1fr)_minmax(19rem,0.86fr)]">
        <ProductSection labelledBy="personalisation-options-heading" className="overflow-hidden">
          ...
          <dl className="divide-y divide-[color:var(--border)] border-t border-[color:var(--border)]">
            {[
              ...
              <div
                key={label}
                className="grid min-h-tap w-full gap-1 px-5 py-4 sm:grid-cols-[minmax(10rem,0.72fr)_minmax(0,1.28fr)_auto] sm:items-center sm:gap-3"
              >
...
function ReviewStage({ onBack, onActivated }: { onBack: () => void; onActivated: () => void }) {
  ...
  return (
    <div className="space-y-5" data-testid="caring-contact-screen-review-activation">
      ...
      <div className="grid min-w-0 gap-5 lg:grid-cols-[minmax(0,1.05fr)_minmax(20rem,0.95fr)]">
        <MessagePreviewCard />
      ...
      <div className="flex flex-col-reverse gap-3 border-t border-[color:var(--border)] pt-5 sm:flex-row sm:justify-between">
        ...
        <Button variant="primary" icon={CheckCircle2} onClick={() => setConfirmOpen(true)}>
