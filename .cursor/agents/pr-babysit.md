...
5. Handle review threads per [Review threads](../../docs/agents/pull-request-workflow.md#review-threads): validate findings against the current head, fix clear P0/P1 and scoped P2s, reply before resolving with the direct resolution tool, and leave ambiguous or product-sensitive threads open for Josh.
...
9. Never merge into `main`, force-push, close the PR, enable/disable auto-merge, or delete branches unless the user explicitly asks. Per-PR auto-merge state is user-owned: automation must not disable or re-enable it. If auto-merge is already armed, an ordinary fast-forward push to fix CI or a review thread may still proceed — GitHub re-validates required checks against the new head before it merges. A force-push, history rewrite, or base/target change while armed stays frozen until the PR merges or the user manually changes that state. Merging a `supabase/` change is a live clinical-database deploy — see [Merge authority](../../docs/agents/pull-request-workflow.md#merge-authority).
10. **Follow CI while it is useful, then stop**, per [Follow CI](../../docs/agents/pull-request-workflow.md#follow-ci). Never park a cron job or other open-ended watch on the PR.
11. Follow `docs/codex-review-protocol.md` and record every completed review or sweep — including pure and no-op reviews — per [Records](../../docs/agents/pull-request-workflow.md#records).

Report before/after: merge state, CI, threads fixed vs left open, commits pushed, and any remaining human decision. Once CI settles (or a run is not this PR's to fix), state plainly where CI stands (green, red with the failing check named, or still running).
