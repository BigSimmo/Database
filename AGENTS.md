...
- Live-mode wiring in this VM: the non-secret public identifiers (`NEXT_PUBLIC_SUPABASE_URL`, `SUPABASE_PROJECT_REF`, `SUPABASE_PROJECT_NAME`, `RAG_PROVIDER_MODE=auto`) live in a gitignored `/workspace/.env.local` (persists in the snapshot). The actual secrets (`SUPABASE_SERVICE_ROLE_KEY`, `OPENAI_API_KEY`, and — for browser sign-in / authenticated Playwright — `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY`, `E2E_USER_EMAIL`, `E2E_USER_PASSWORD`) must be Cloud Agent Secrets; do not put them in `.env.local`. In dev, `isDemoMode()` only needs the URL + service-role key + matching ref, so the server-side answer/retrieval path goes live even before the publishable key is added (the publishable key gates browser auth and live E2E). Confirm with `curl -s localhost:<port>/api/setup-status` (every check `ready` = live).
- **tmux stale-environment gotcha (important):** newly injected Cloud Secrets only appear in shells/processes started _after_ injection. A long-lived tmux server started earlier keeps its old env, so a dev server relaunched in an old tmux pane will still see the keys as missing and stay in demo mode even with `.env.local` present. After secrets change, start the dev server from a shell that has them — recreate the tmux server (`tmux -f /exec-daemon/tmux.portal.conf kill-server` then a fresh `new-session`) or verify with `printenv SUPABASE_SERVICE_ROLE_KEY` in the pane before `npm run dev`.
- Full-access toolchain (provisioned in the VM snapshot, not by the update script): Deno 2 (symlinked into `/usr/local/cargo/bin` alongside `node`/`npm`, so `npm run check:edge:functions` works), Tesseract OCR + the Python worker stack from `worker/python/requirements.txt` (PyMuPDF, pytesseract, medspaCy) installed into the system `python3` so `PYTHON_BIN` unset resolves correctly, and the Playwright Chromium/Firefox/WebKit browsers + their system libs. The update script only refreshes the repo-file-tied deps (`npm ci` and the `worker/python/requirements.txt` pip refresh into the user site). If a pulled change bumps versions, re-provision manually: `npm i -g deno@2 && ln -sf "$(npm root -g)/deno/deno" /usr/local/cargo/bin/deno`; `sudo apt-get install -y tesseract-ocr`; `sudo python3 -m pip install --break-system-packages -r worker/python/requirements.txt`; `sudo env PATH="$PATH" ./node_modules/.bin/playwright install --with-deps chromium firefox webkit`.
- Playwright must run through the repo scripts (`npm run test:e2e*`, `npm run verify:ui`), never the raw `playwright` CLI — `scripts/playwright-base-url.ts` hard-fails a direct CLI invocation because the runner owns an isolated production server (it builds `.next-playwright` and serves on its own port, separate from the `npm run dev` port). `verify:ui` runs `check:runtime` + `check:installed-lock-parity` + `test:e2e:pr` (Chromium) and needs `npm run ensure` first.
- Still needs live Supabase to actually do work even with the toolchain present: `npm run worker` connects to Supabase to claim ingestion jobs, and `check:supabase-project`/`verify:release`/live evals call providers. `verify:release` additionally runs governance/eval gates. Treat missing-secret failures of these in demo mode as expected, not regressions.
