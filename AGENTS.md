...
## Bare PR publication is not readiness work
  ...
  - If a local commit or push hook is the only blocker, publish with `git commit --no-verify` and `git push --no-verify`; do not spend time preparing dependencies or formatting solely to satisfy the hook. This exception is limited to the explicit bare-publication request and does not weaken normal-push safeguards.
