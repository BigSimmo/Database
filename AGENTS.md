...
- **Selective PR previews.** Prefer Railway Focused PR Environments with the existing service watch paths: preview affected runtime services and their dependencies, skip documentation/test-only changes, and keep production deployment from `main`. Follow [the preview policy and activation steps](docs/deployment-architecture.md#selective-railway-pr-previews). The dashboard setting requires separate activation; a documentation PR does not enable it. Do not weaken required CI checks or change Supabase branching to implement this Railway rule.
