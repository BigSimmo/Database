...
- Context7 peer-library docs habit (and the Next 16 local-docs carve-out) lives in `docs/agents-guide.md`. Cloud Agent Secrets may put `CONTEXT7_API_KEY` in agent `process.env`, but that does not authenticate the host-injected Context7 MCP connector or expand desktop `.cursor/mcp.json` `${env:}` by itself.
