...
Runnable study at [`/mockups/phone-mode-sheet-yes`](../src/app/mockups/phone-mode-sheet-yes/page.tsx): design review plus **YES 01 dense glance list** (flat title-only catalogue, no Find/Diagnose/Care lanes — shipping recommendation). Organised icon-deck is shown as a rejected contrast. Shared mockup chrome is suppressed so only the in-frame sheet is judged.
