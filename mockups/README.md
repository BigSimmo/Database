...
Runnable study at [`/mockups/phone-mode-sheet-yes`](../src/app/mockups/phone-mode-sheet-yes/page.tsx): design review of the shipping phone mode sheet plus two large YES comps (sectioned clinical list, icon deck with lane chips). Shared mockup chrome is suppressed so only the in-frame sheet is judged.
