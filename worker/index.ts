...
async function nextJob() {
  const { data, error } = await supabase
    .from("ingestion_jobs")
    .select("id,document_id,documents(id,title,file_name,file_type,storage_path)")
    .eq("status", "pending")
...
async function uploadAndCaptionImages(
  ...
  for (const [index, image] of extracted.images.entries()) {
    ...
    const imagePath = `local/${job.document_id}/image-${index + 1}${ext}`;
...
async function insertEmbeddedChunks(job: JobRow, extracted: ExtractedDocument) {
  const pagesByNumber = new Map(
    extracted.pages.map((page) => [page.pageNumber, page.text] as const),
  );
  ...
  const chunks = buildChunks(
    extracted.pages.map((page) => ({
      ...
      pageText: page.text,
      images: insertedImages.filter(
        (image) => image.pageNumber === null || image.pageNumber === page.pageNumber,
      ),
    })),
  );
...
async function main() {
  ...
  while (true) {
    ...
    if (job) {
      console.log(`Processing ${job.documents.file_name} (${job.id})`);
