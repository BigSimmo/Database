...
// on home" gates the SharedHomeEmptyState chips, "Compact citations" drops the
...
describe("recent searches on home preference", () => {
  it("shows recent-query chips when the preference is on (default)", () => {
    ...
    renderEmptyState();
    expect(screen.getByTestId("shared-home-recent-queries")).toBeInTheDocument();
  ...
  it("hides recent-query chips when the preference is off", () => {
    ...
    renderEmptyState();
    expect(screen.queryByTestId("shared-home-recent-queries")).not.toBeInTheDocument();
