...
describe("Storage buckets migration chain (schema-drift fix)", () => {
  // 20260527000000_bulk_ingestion.sql created storage.objects RLS policies that
  // reference clinical-documents/clinical-images, but only schema.sql (not the
  // migration chain) ever created the buckets themselves. A database built purely
  // by replaying migrations therefore had policies pointing at buckets that did
  // not exist. This migration backfills the bucket creation into the chain.
  const createStorageBucketsMigration = readFileSync(
    new URL("../supabase/migrations/20260717130000_create_storage_buckets.sql", import.meta.url),
    "utf8",
  ).replace(/\s+/g, " ");

  it("creates the private clinical-documents bucket with schema.sql's size limit and mime allowlist", () => {
    expect(createStorageBucketsMigration).toContain(
      "insert into storage.buckets (id, name, public, file_size_limit, allowed_mime_types)",
    );
    expect(createStorageBucketsMigration).toContain("'clinical-documents', 'clinical-documents', false, 157286400,");
    expect(createStorageBucketsMigration).toContain("'application/pdf'");
    expect(createStorageBucketsMigration).toContain(
    ...
    expect(createStorageBucketsMigration).toContain("'text/plain'");
  });

  it("creates the private clinical-images bucket with schema.sql's size limit and mime allowlist", () => {
    expect(createStorageBucketsMigration).toContain("'clinical-images', 'clinical-images', false, 52428800,");
    expect(createStorageBucketsMigration).toContain("array['image/png', 'image/jpeg', 'image/webp']");
  });

  it("uses byte-precise size limits (150 MB documents, 50 MB images)", () => {
    expect(createStorageBucketsMigration).toContain(String(150 * 1024 * 1024));
    expect(createStorageBucketsMigration).toContain(String(50 * 1024 * 1024));
  });

  it("upserts idempotently, re-affirming public = false on conflict for both buckets", () => {
    const marker = "on conflict (id) do update set public = false;";
    const onConflictCount = createStorageBucketsMigration.split(marker).length - 1;
    expect(onConflictCount).toBe(2);
    const insertCount = createStorageBucketsMigration.split("insert into storage.buckets").length - 1;
    expect(insertCount).toBe(2);
  });

  it("never flips a bucket public (regression: on-conflict must not drop the private ACL)", () => {
    expect(createStorageBucketsMigration).not.toContain("public = true");
    expect(createStorageBucketsMigration).not.toContain(
      "'clinical-documents', 'clinical-documents', true,",
    );
    expect(createStorageBucketsMigration).not.toContain("'clinical-images', 'clinical-images', true,");
  });

  it("mirrors schema.sql's storage.buckets inserts byte-for-byte", () => {
    const extractBucketInserts = (text: string) => {
      const marker = "on conflict (id) do update set public = false;";
      const start = text.indexOf("insert into storage.buckets");
      expect(start, "storage.buckets insert not found").toBeGreaterThanOrEqual(0);
      const firstEnd = text.indexOf(marker, start);
      expect(firstEnd, "first bucket upsert not found").toBeGreaterThan(start);
      const secondStart = text.indexOf("insert into storage.buckets", firstEnd);
      expect(secondStart, "second bucket insert not found").toBeGreaterThan(firstEnd);
      const secondEnd = text.indexOf(marker, secondStart);
      expect(secondEnd, "second bucket upsert not found").toBeGreaterThan(secondStart);
      return text.slice(start, secondEnd + marker.length);
    };
    expect(extractBucketInserts(createStorageBucketsMigration)).toBe(extractBucketInserts(schema));
  });

  it("creates the exact bucket ids the storage.objects RLS policies depend on", () => {
    // Regression guard for the drift bug itself: the policy migration references
    // these bucket_ids, so the bucket-creation migration must define the same ones.
    ...
    expect(createStorageBucketsMigration).toContain("'clinical-documents',");
    expect(createStorageBucketsMigration).toContain("'clinical-images',");
  });
});
