...
/** Blob table keyed `<ref>:<path>`; a missing key means the path does not exist. */
...
describe("merge-loss classification", () => {
  it("does not flag a file whose change is still present", () => {
    const blobAt = reader({ "pre:kept.ts": "aaa", "head:kept.ts": "bbb" });
    expect(classifyMergeLoss({ ref: "head", blobAt, landings: [landing(1, ["kept.ts"])] }).findings).toEqual([]);
  });

  it("flags a file that reverted to its pre-merge blob", () => {
    // The acf78bf case: the landing's contribution to this file is gone.
    const blobAt = reader({ "pre:lost.ts": "ccc", "head:lost.ts": "ccc" });
    const { findings } = classifyMergeLoss({ ref: "head", blobAt, landings: [landing(1803, ["lost.ts"])] });
  ...
  it("flags a file the pull request added that is absent again", () => {
    ...
    const { findings } = classifyMergeLoss({ ref: "head", blobAt: reader({}), landings: [landing(1, ["added.ts"])] });
  ...
  it("does not flag a deletion that survived", () => {
    ...
    const blobAt = reader({ "pre:removed.ts": "ddd" });
    expect(classifyMergeLoss({ ref: "head", blobAt, landings: [landing(1, ["removed.ts"])] }).findings).toEqual([]);
  });
  ...
  it("orders findings by how much of the landing is missing", () => {
    const blobAt = reader({ "pre:a.ts": "1", "head:a.ts": "1", "pre:b.ts": "2", "head:b.ts": "2" });
...
describe("merge-loss reconciliation exemption", () => {
  ...
  it("does not report an inbox request that reconcile moved to applied/", () => {
    ...
    const result = classifyMergeLoss({
      ref: "head",
      blobAt: reader({ [`head:${applied}`]: "eee" }),
  ...
  it("still reports an inbox request that vanished without an audit record", () => {
    ...
    const result = classifyMergeLoss({ ref: "head", blobAt: reader({}), landings: [landing(1915, [request])] });
    expect(result.findings).toHaveLength(1);
    expect(result.filesExempted).toBe(0);
  });

  it("credits the move only for a matching request filename", () => {
    const blobAt = reader({ [`head:${applied}`]: "eee" });
    expect(isReconciliationMove(request, "head", blobAt)).toBe(true);
    expect(isReconciliationMove(`${INBOX}/22222222-2222-4222-8222-222222222222.json`, "head", blobAt)).toBe(false);
    expect(isReconciliationMove("src/lib/rag/rag.ts", "head", blobAt)).toBe(false);
    expect(isReconciliationMove(applied, "head", blobAt)).toBe(false);
  });
});
