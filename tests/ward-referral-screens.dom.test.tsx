 ...
 */
const CONDITIONAL_QUESTION_NAMES = ["Emergency department", "Community team"] as const;
...
describe("ReferralBoard", () => {
  it("renders exactly the real fixture's two queued referrals, in urgency-then-longest-wait order", () => {
    ...
    expect(ids).toEqual(["RF-001", "RF-009", "RF-005", "RF-011"]);
  });
  ...
  it("the decided section says plainly that an acceptance holds no bed and creates no movement", () => {
    ...
    expect(note).toHaveTextContent(/no bed is pulled/i);
...
const SEEDED_REFERRAL_IDS = new Set(referrals.map((referral) => referral.id));
...
describe("ReferralBoard — recently decided caps at ten (owner ruling, 2026-09-02)", () => {
  it("shows exactly the ten most recently decided referrals, never the first ten in array order", () => {
    ...
    // Build eleven decided referrals total: the fixture's own seven, plus four raised-and-declined
    // here. Each pair of clicks fully decides one new referral (raise, then decline the one just
    // raised) before the next pair starts.
    for (let i = 0; i < 4; i += 1) {
    ...
    // The one referral old enough to be dropped: RF-010, decided 24 days before the anchor, older
    // than every other decided referral in play (the fixture's other six, and all four raised
    // here, which tie for most recent). Everything else must still be on screen.
    expect(screen.queryByTestId("ward-referral-board-decided-row-RF-010")).not.toBeInTheDocument();
    for (const id of ["RF-002", "RF-003", "RF-004", "RF-006", "RF-007", "RF-008"]) {
...
function RaiseRefuseThenAcceptHarness({ acceptedBy }: { acceptedBy: "psychiatric_ward" | "emergency_department" }) {
  ...
  function refuse(destinationKind: "emergency_department" | "community_team", reason: ReferralDeclineReason) {
    if (!created) return;
    dispatch({
      type: "DECLINE_REFERRAL",
  ...
  return (
    <div>
      ...
      <button
        ...
        onClick={() => refuse("community_team", "referred_elsewhere")}
      >
...
describe("ReferralBoard — an acceptance does not erase a refusal", () => {
  ...
  it("adds no second refusals line to a row whose refusals are already the outcome, or to one with none", () => {
    ...
    expect(screen.getByTestId("ward-referral-board-decided-detail-RF-004").textContent).toBe(
      `Psychiatric ward: ${DECLINE_REASON_LABELS[declineReason]}`,
    );
  });
});
