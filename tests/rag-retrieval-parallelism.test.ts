...
describe("retrieval hydration parallelism", () => {
  it("overlaps only independent metadata and memory reads before deterministic assembly", () => {
    ...
    const helper = source.slice(
      source.indexOf("async function hydrateCandidatesWithMetadataAndMemory"),
    ...
    expect(helper.indexOf("applyMemoryBoostArtifacts(")).toBeGreaterThan(helper.indexOf("await Promise.all(["));
  });
