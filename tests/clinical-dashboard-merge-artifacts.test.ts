...
describe("ClinicalDashboard merge-artifact guards", () => {
  ...
  it("releases the Safari toolbar reserve only after phone composers hide", () => {
    ...
    // DocumentViewer keeps a small 0.75rem resting pad even once the composer
    // hides, so the last card never paints flush against the physical bottom
    // edge (see the comment on document-viewer-content in DocumentViewer.tsx).
    expect(documentViewerSource).toContain('"max-sm:pb-[0.75rem]"');
