...
describe("ClinicalDashboard merge-artifact guards", () => {
  ...
  it("releases the Safari toolbar reserve only after phone composers hide", () => {
    ...
    expect(globalSearchShellSource).toContain('bottomComposerHidden ? "max(0.75rem, var(--safe-area-bottom))"');
    ...
    expect(clinicalDashboardSource).toContain('bottomComposerHidden ? "max(0.75rem, var(--safe-area-bottom))"');
