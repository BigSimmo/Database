...
describe("consolidated mode home redirects", () => {
  ...
  /*
   * An unsubmitted `<mode>/search` has nothing to show for these four: their
   * component falls back to the mode home, which is the page consolidation
   * retired. Resolved in the proxy so it is a 307 rather than the streamed meta
   * refresh a page-level redirect would emit.
   */
  it("forwards an unsubmitted mode search to the shared home", () => {
    ...
    expect(search("/formulation/search")).toBe("/?mode=formulation");
    expect(search("/specifiers/search")).toBe("/?mode=specifiers");
    expect(search("/calculators/search")).toBe("/?mode=calculators");
    expect(search("/specifiers/search", "q=+++")).toBe("/?mode=specifiers");
  });

  /*
   * The scope is deliberately narrow. These three are linked from their own mode
   * nav with no query at all (`modeSecondaryNavigationRegistry`), so they are
   * browse surfaces: redirecting them would break the tab pointing at them.
   */
  it("leaves query-free browse surfaces alone", () => {
    const search = (pathname: string, query = "") => unsubmittedModeSearchTarget(pathname, new URLSearchParams(query));

    for (const pathname of ["/factsheets/search", "/dictionary/search", "/therapy-compass/search"]) {
