...
const OWNER_SCOPED_API_TABLES = new Set([
  ...
  // The three CME tables the API routes query directly. `cme_requirements` and
  // `cme_routines` are absent on purpose: nothing under src/app/api reaches them
  // yet, and this list must match what the routes actually touch, not what the
  // schema contains — an aspirational entry here would make the guard pass over
  // a table no route has ever been checked against.
  "cme_allocations",
  "cme_entries",
  "cme_years",
