...
describe("design-system adoption manifest", () => {
  ...
  it("is deterministic and records declared global v2 adoption", { timeout: 90_000 }, () => {
    ...
    // 86 = 59 + 6 + 13 + 8: the 59 production pages that preceded both changes,
    ...
    // detail route, and the retired constellation redirect), and the eight Caring
    // Contacts workspace screens built so far (Today, the Patients caseload added by
    // Phase 2B Task 5, the per-patient overview added by Task 6, the activation
    // wizard added by Task 7, the templates library added by Task 15, Guidance and
    // Reports added by Task 19, and the per-version template detail added by Task 16).
    // Redirect stubs keep legacy deep links resolving and still count as declared
    // routes. This is a census, so a route nobody intended to add still fails the
    // contract.
    expect(manifest.routeCoverage.discovered).toHaveLength(86);
