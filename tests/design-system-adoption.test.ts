...
describe("design-system adoption manifest", () => {
  ...
  it("is deterministic and records declared global v2 adoption", { timeout: 90_000 }, () => {
    ...
    // 81 = 59 + 6 + 13 + 2 + 1: the 59 production pages that preceded both changes,
    // the six `<mode>/search` routes home consolidation split out of the bare paths,
    // the thirteen-route Ward Flow synthetic patient-flow prototype, the two Caring
    // Contacts workspace screens (Today and the Phase 2B Patients caseload), and
    // `/factsheets/topics`. Redirect stubs keep legacy deep links resolving and still
    // count as declared routes. This is a census, so a route nobody intended to add
