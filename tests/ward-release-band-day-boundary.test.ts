 ...
 */
/*
 * NO CAST. This literal was `as unknown as BedRelease` until 2026-09-02, and the cast was hiding
 * NINE discrepancies against the real type: six required fields absent (`waitingOn`, `blocker`,
 * `blockedBy`, `preparing`, `preparationNote`, `confirmedBy`) and three fields the model does not
 * have at all (`blocked`, `blockReason`, `basis`). `blocked`/`blockReason` are the pre-2026-08-28
 * shape, from when `blocked` was a fourth STATE; the field is now `blocker`, and it is the one the
 * absent/phantom pair lines up on — so this file was banding discharges on a record where a block
 * could neither be set nor read, and the system can never produce it.
 *
 * That is worse than an untidy literal. `releaseBand` is what a ward reads to plan a bed, and a
 * test that proves it against an impossible record proves nothing about the real one. Three chats
 * found this independently and none of them owned the file.
 *
 * Every field below is now supplied honestly and the compiler enforces it: add a twelfth field to
 * `BedRelease` and this stops compiling, which is exactly what the cast was suppressing. Values
 * match the established honest literal in `tests/ward-bed-availability.test.ts:15`.
 */
function aRelease(expectedAt: number): BedRelease {
  return {
    ...
    state: "expected",
    expectedAt,
    // Non-null because `state` is "expected"; "Nothing outstanding" is a real value meaning an
    // expected discharge with no obstacle, not an absence. Banding must not depend on it.
