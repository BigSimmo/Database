...
describe("notice stack waits for the app shell", () => {
  // `#TYZK23`. Every phone geometry rule for this component selects on
  // `body:has(#main-content[data-phone-footer-owner="hero"])`, and this
  // component renders from the ROOT layout — outside the route's Suspense
  // boundary — so on `/` it can paint while the page is still the
  // src/app/loading.tsx skeleton and `#main-content` does not exist. The
  // selector then flips from false to true and restyles a fixed, already
  // painted card. CI attributed the whole 0.223 mobile-root breach to
  // `body.min-h-full > div.pwa-notice-stack` (run 32526303319), and the
  // bistability is only whether those two land in the same frame.
  ...
  it("stays unmounted until #main-content arrives, then paints once", async () => {
    setReadyState("loading");
    Object.defineProperty(navigator, "onLine", { configurable: true, value: false });
    try {
      render(<PwaLifecycle />);

      // Skeleton stage: painting here is what the flip would later move.
      expect(screen.queryByRole("region", { name: "You’re offline" })).not.toBeInTheDocument();
  ...
  it("releases on load so a surface that renders no #main-content still gets notices", async () => {
    setReadyState("loading");
    Object.defineProperty(navigator, "onLine", { configurable: true, value: false });
    try {
      render(<PwaLifecycle />);
      ...
      // The streamed document is complete before `load` fires, so an absent
      // `#main-content` at this point is absent for good and the selector can
      // no longer flip — releasing here cannot reintroduce the shift.
      await act(async () => {
        setReadyState("complete");
