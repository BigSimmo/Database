...
test.describe("@mockup Ward referrals — the front door, phone to board to accepted", () => {
  ...
  test("a referral raised on the phone-width intake form reaches the coordinator's board, matches against the network, and is accepted — with the board reflecting every step without a reload", async ({
    ...
    // not find. Read from referral-board.tsx:472, which renders "No bed is pulled, no movement is
    // created…". ⚠️ NINETEENTH CAUSE — the triage's six were derived from one run's failure list,
    // and this test fails LATER in the same journey, so its own failure was masked until the
    // earlier ones were repaired. A failure list is a snapshot of what fails FIRST.
