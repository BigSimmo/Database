...
const readySetupChecks = [
  {
    ...
    status: "ready",
    detail: "Test environment ready.",
  },
  {
    id: "project",
    label: "Clinical KB Database target",
    status: "ready",
    detail: "Test Supabase project ready.",
  },
  {
    id: "schema",
    label: "supabase/schema.sql applied",
    status: "ready",
    detail: "Test schema ready.",
  },
  {
    id: "search",
    label: "Search RPC and vector indexes",
    status: "ready",
    detail: "Test search schema ready.",
  },
  {
    id: "openai",
    label: "OpenAI API key available",
    status: "ready",
    detail: "Test OpenAI ready.",
  },
  {
    id: "worker",
    label: "npm run worker running",
    status: "unknown",
    detail: "Worker not required for UI smoke.",
  },
];
...
async function submitDifferentialSearch(page: Page, query: string) {
  const input = page
    .locator('input[placeholder="Ask or search a presentation"]:visible')
    .first();
  const submit = page.locator(
    'button[aria-label="Search differential presentations"]:visible',
  );
  ...
  await Promise.all([
    waitForDifferentialCatalogQuery(page, query),
    submit.click(),
  ]);
}
...
async function mockAnswerDashboardApi(page: Page) {
  ...
  await page.route(/\/api\/medications(?:\?.*)?$/, async (route) => {
    ...
    const matches = query
      ? rankMedicationRecords(records, query, limit)
      : undefined;
    await route.fulfill({
      json: {
        records,
  ...
  await page.route(/\/api\/answer(?:\/stream)?(?:\?.*)?$/, async (route) => {
    ...
    const answer = demoAnswer(
      body.query ?? "What monitoring is required?",
      body.documentId,
      body.documentIds,
    );
  ...
  await page.route(
    ...
    async (route) => {
      ...
      if (!record) {
        await route.fulfill({
          status: 404,
          json: { error: "Registry record not found" },
        });
        return;
      }
      await route.fulfill({
...
async function commandSurfaceOpensAbovePill(page: Page) {
  ...
  expect(
    geometry?.dropdownBottom ?? Number.POSITIVE_INFINITY,
  ).toBeLessThanOrEqual((geometry?.pillTop ?? 0) + 2);
}
...
async function waitForReactEventHandler(
  locator: Locator,
  eventName: "onChange" | "onClick" | "onSubmit" = "onClick",
) {
  await expect
    .poll(
      async () =>
  ...
  return visibleGlobalSearchInput(page)
    ...
    .evaluate((input, homeTestId) => {
      ...
      const home = homeTestId
        ? [...document.querySelectorAll(`[data-testid="${homeTestId}"]`)].find(
            (candidate) => {
      ...
      for (
        let node: HTMLElement | null = form.parentElement;
        node && !stickyAncestor;
      ...
      return {
        ...
        insideDesktopPageSlot: Boolean(
          form.closest('[data-testid="desktop-page-search-composer-slot"]'),
        ),
...
async function expectVerticalSeparation(
  ...
  const metrics = await page.evaluate(
    ({ upperSelector, lowerSelector }) => {
      const upper = document
        .querySelector(upperSelector)
        ?.getBoundingClientRect();
      const lower = document
        .querySelector(lowerSelector)
        ?.getBoundingClientRect();
  ...
  expect(
    (metrics?.lowerTop ?? 0) - (metrics?.upperBottom ?? 0),
...
test.describe("Clinical KB tools launcher", () => {
  ...
  for (const viewport of [
    ...
    test(`tools launcher is usable at ${viewport.name}`, async ({ page }) => {
      ...
      await expect(
        page.getByRole("region", { name: "Quick tool shortcuts" }),
      ).toBeVisible();
      await expect(
        page.getByRole("heading", { name: "All tools" }),
      ).toBeVisible();
      await expect(page.locator("#launcher-results-panel")).toHaveAttribute(
        "role",
        "group",
      );
      await expect(page.locator("#launcher-results-panel")).toHaveAttribute(
      ...
      if (viewport.name === "mobile") {
        ...
        await page
          .getByRole("radiogroup", { name: "Category" })
          .getByRole("radio", { name: "All tools" })
          .click();
        await page
          .getByTestId("application-row-medication-prescribing")
          .click();
        const selectedSheet = page.getByRole("dialog", {
          name: "Medication Prescribing",
        });
        await expect(selectedSheet).toBeVisible();
        await expect(
          selectedSheet.getByRole("heading", {
            name: "Medication Prescribing",
          }),
        ).toBeVisible();
        const mobileLaunchLink = selectedSheet
        ...
        await page
          .getByRole("button", { name: "Close Medication Prescribing" })
          .click();
        await expect(selectedSheet).toBeHidden();
      } else {
        await expect(
          page.getByRole("button", {
            name: "View details for Clinical KB Search",
          }),
        ).toBeVisible();
      }
      ...
      if (viewport.name === "mobile") {
        ...
        await expect(
          page.getByTestId("tools-home").getByTestId("global-search-input"),
        ).toBeVisible();
        await expect(
          page.locator("form.answer-footer-search-dock"),
        ).toHaveCount(0);
      } else {
        await expect(
          page.getByTestId("tools-home").getByTestId("global-search-input"),
  ...
  test("standalone tools route uses the shared global search", async ({
    page,
  }) => {
    ...
    await expect(
      page.getByTestId("application-card-medication-prescribing"),
    ).toBeVisible();
    await expect(page.getByTestId("application-card-documents")).toBeHidden();
    await expectNoPageHorizontalOverflow(page);
  });

  test("tool descriptions remain complete across supported breakpoints", async ({
    page,
  }) => {
    await gotoLauncher(page, "/tools");
    ...
    for (const width of [320, 390, 639, 768, 1440, 1920]) {
      ...
      const description = tool
        .getByText(
          "Ask source-backed clinical questions and move straight to evidence.",
          {
            exact: true,
          },
        )
  ...
  test("launcher links point to the expected in-app modes", async ({
    page,
  }) => {
    await page.setViewportSize({ width: 1280, height: 900 });
    await gotoLauncher(page);
    ...
    await expect(
      page.locator('a[href^="http://localhost"], a[href^="http://127.0.0.1"]'),
    ).toHaveCount(0);
  });

  test("search and filters reduce visible application rows without overflow", async ({
    page,
  }) => {
    await page.setViewportSize({ width: 1280, height: 900 });
    await gotoLauncher(page);
  ...
  test("tools mode embeds the launcher content inside the dashboard", async ({
    page,
  }) => {
    await page.setViewportSize({ width: 1280, height: 900 });
    ...
    await expect(
      page.getByRole("button", { name: "Mode Tools" }),
    ).toBeVisible();
    await expect(
      page.locator('input[placeholder="Search tools..."]:visible').first(),
    ).toHaveValue("medication");
    ...
    await expect(
      toolsHub.getByRole("heading", { level: 1, name: "Tools" }),
    ).toBeVisible();
    ...
    await expect(
      queryRibbon.getByRole("heading", { name: "medication" }),
    ).toBeVisible();
    await expect(
      queryRibbon.getByRole("group", { name: "Filter tools by category" }),
    ).toBeVisible();
    const medicationDetails = toolsHub.getByRole("button", {
      name: "View details for Medication Prescribing",
    });
    await expect(medicationDetails).toHaveAttribute("aria-haspopup", "dialog");
    await expect(
      toolsHub.getByTestId("application-card-documents"),
    ).toBeHidden();
    await expect(
      toolsHub.getByTestId("tool-mode-result-medications"),
    ).toHaveCount(0);

    await medicationDetails.click();
    const medicationDialog = page.getByRole("dialog", {
      name: "Medication Prescribing",
    });
    await expect(medicationDialog).toBeVisible();
    const medicationLaunch = medicationDialog
      .locator('a[href="/medications"]')
      .first();
  ...
  test("shared-home presentation updates in place when the mode changes", async ({
    page,
  }) => {
    await page.setViewportSize({ width: 1280, height: 900 });
    ...
    await page.addInitScript(() =>
      window.localStorage.setItem("clinical-kb-sidebar-collapsed", "1"),
    );
    ...
    await expect(sharedHomeBrand).toContainText(
      "Source-backed clinical search",
    );
    await expect(
      sharedHome.getByRole("heading", { level: 2, name: "Clinical Answers" }),
    ).toBeVisible();
    await expect(sharedHome.locator(".mode-home-icon svg")).toHaveClass(
      /\blucide-sparkles\b/,
    );
    ...
    const servicesMode = answerMenu.getByRole("menuitemradio", {
      name: /^Services\b/,
    });
    ...
    const servicesModeButton = page.getByRole("button", {
      name: "Mode Services",
    });
    ...
    await expect(
      sharedHome.getByRole("heading", { level: 2, name: "Clinical Services" }),
    ).toBeVisible();
    await expect(sharedHome.locator(".mode-home-icon svg")).toHaveClass(
      /\blucide-route\b/,
    );
    await expect(sharedHomeBrand).toContainText("Clinical KB");
    await expect(
      page.getByText("Services Navigator", { exact: true }),
    ).toHaveCount(0);
    ...
    await expect(
      servicesMenu.getByRole("menuitemradio", { name: /^Answer\b/ }),
    ).toBeVisible();
    await expect(
      servicesMenu.getByRole("menuitemradio", { name: /^Documents\b/ }),
    ).toBeVisible();
    await expect(
      servicesMenu.getByRole("menuitemradio", { name: /^Services\b/ }),
    ).toBeVisible();
    await expect(
      servicesMenu.getByRole("menuitemradio", { name: /^Forms\b/ }),
    ).toBeVisible();
    await expect(
      servicesMenu.getByRole("menuitemradio", { name: /^Differentials\b/ }),
    ).toBeVisible();
    await expect(
      servicesMenu.getByRole("menuitemradio", { name: /^Medication\b/ }),
    ).toBeVisible();
    await expect(
      servicesMenu.getByRole("menuitemradio", { name: /^Tools\b/ }),
    ).toBeVisible();
    const formsMode = servicesMenu.getByRole("menuitemradio", {
      name: /^Forms\b/,
    });
    ...
    await expect(
      sharedHome.getByRole("heading", { level: 2, name: "Clinical Forms" }),
    ).toBeVisible();
    await expect(sharedHome.locator(".mode-home-icon svg")).toHaveClass(
      /\blucide-file-pen-line\b/,
    );
  ...
  test("submitting from the shared home opens the selected mode's search page", async ({
    page,
  }) => {
    await page.setViewportSize({ width: 1280, height: 900 });
    await gotoLauncher(page, "/?mode=answer");
    ...
    await expect(page).toHaveURL(/\/dsm\/search\?.*q=bipolar/, {
      timeout: 20_000,
    });
    await expect(visibleGlobalSearchInput(page)).toHaveCount(1);
    await expectNoPageHorizontalOverflow(page);
  ...
  test("dashboard mode switches return answer results to the shared home as a draft", async ({
    page,
  }) => {
    await page.setViewportSize({ width: 1280, height: 900 });
    ...
    await expect(
      page.getByRole("button", { name: "Mode Forms" }),
    ).toBeVisible();
    await expect(
      visibleByTestId(page, "shared-home-empty-state"),
    ).toBeVisible();
  ...
  test("header mode switches return results and mode homes to the shared home", async ({
    page,
  }) => {
    await page.setViewportSize({ width: 1280, height: 900 });
    ...
    await expect(
      page.getByRole("button", { name: "Mode Services" }),
    ).toBeVisible();
    await expect(
      page.getByText("Services Navigator", { exact: true }),
    ).toBeVisible();
    ...
    await expect(
      page.getByTestId("service-search-result-13yarn"),
    ).toBeVisible();
    ...
    await expect(page).toHaveURL(/\/\?mode=forms&q=13YARN$/, {
      timeout: 20_000,
    });
    await expect(
      page.getByRole("button", { name: "Mode Forms" }),
    ...
    const currentFormsMode = menu.getByRole("menuitemradio", {
      name: /^Forms\b/,
    });
  ...
  test("phone shared-home presentation stays accessible and centered after a mode change", async ({
    page,
  }) => {
    await mockAnswerDashboardApi(page);
    ...
    const answerMenu = await openAppModeMenu(page, "Answer");
    const specifiersMode = answerMenu.getByRole("menuitemradio", {
      name: /^Specifiers\b/,
    });
    ...
    const specifiersModeButton = page.getByRole("button", {
      name: "Mode Specifiers",
    });
    await expect(specifiersModeButton).toBeVisible();
    await expect(specifiersModeButton).toBeFocused();
    await expect(
      sharedHome.getByRole("heading", {
        level: 2,
    ...
    await expect(heroSearch).toHaveAttribute(
      "placeholder",
    ...
    const expectSoundPhoneGeometry = async () => {
      ...
      expect(
        Math.abs((metrics?.formCenterX ?? 0) - (metrics?.homeCenterX ?? 0)),
    ...
    await expectSoundPhoneGeometry();
    await expect(
      page.locator(".answer-footer-search-chip:visible"),
    ).toHaveCount(0);
    // The home hero is the only phone surface with the APP-5 privacy notice.
    await expect(
      page.getByTestId("answer-composer-privacy-warning"),
    ).toBeVisible();
  });

  test("320px shared-home presentation wraps the longest mode copy without overflow", async ({
    page,
  }) => {
    await mockAnswerDashboardApi(page);
    ...
    expect(
      (specifiersHeadingBox?.x ?? 0) + (specifiersHeadingBox?.width ?? 0),
    ).toBeLessThanOrEqual(320);
    let metrics = await globalSearchComposerMetrics(
      page,
      "shared-home-empty-state",
    );
    expect(metrics).not.toBeNull();
    ...
    const formulationMode = specifiersMenu.getByRole("menuitemradio", {
      name: /^Formulation\b/,
    });
    ...
    const formulationModeButton = page.getByRole("button", {
      name: "Mode Formulation",
    });
    await expect(formulationModeButton).toBeFocused();
    const formulationModeLabel = formulationModeButton.getByText(
      "Formulation",
      { exact: true },
    );
    await expect(formulationModeLabel).toBeVisible();
    expect(
      await formulationModeLabel.evaluate(
    ...
    expect(
      (formulationHeadingBox?.x ?? 0) + (formulationHeadingBox?.width ?? 0),
    ).toBeLessThanOrEqual(320);
    metrics = await globalSearchComposerMetrics(
      page,
      "shared-home-empty-state",
    );
    expect(metrics).not.toBeNull();
    ...
    expect(metrics?.formWidth ?? 0).toBeLessThanOrEqual(320 - 16);
    expect(
      Math.abs((metrics?.formTop ?? 0) - specifiersComposerTop),
    ).toBeLessThanOrEqual(1);
    await expectNoPageHorizontalOverflow(page);
  });

  test("phone mode homes keep the in-flow hero pill, matching the answer home", async ({
    page,
  }) => {
    await mockAnswerDashboardApi(page);
    ...
    for (const home of [
      ...
      await expectSingleSettledOwner(homeSurface, {
        message: `${home.path} home owner`,
      });
      ...
      await expect(
        page.locator(".universal-command-dropdown:visible"),
      ).toHaveCount(0);
  ...
  for (const viewport of [
    ...
    for (const home of [
      ...
      test(`mode home search composer is present at ${viewport.name} width on ${home.path} @critical`, async ({
        ...
        await expect(
          homeSurface.getByTestId("global-search-input"),
        ).toBeVisible();
      });
    }
  }

  test("answer composer keeps the PHI warning visible before submission @critical", async ({
    page,
  }) => {
    await mockAnswerDashboardApi(page);
    ...
    for (const viewport of [
      ...
      await expect(
        warning.getByRole("link", { name: "Privacy and data processing" }),
      ).toBeVisible();
      await expect(visibleGlobalSearchInput(page)).toHaveAttribute(
        "aria-describedby",
  ...
  for (const home of [
    ...
    },
    {
      ...
      heroTestId: "document-search-empty-state",
    },
    {
      path: "/medications",
      testId: "medication-home",
      heroTestId: "medication-home",
    },
    { path: "/tools", testId: "tools-home", heroTestId: "tools-home" },
    {
      path: "/services",
      testId: "services-home",
      heroTestId: "services-home-template",
    },
    { path: "/forms", testId: "forms-home", heroTestId: "forms-home-template" },
    {
      path: "/differentials",
      testId: "differentials-home",
      heroTestId: "differentials-home-template",
    },
  ] as const) {
    test(`mode home hero uses the shared mobile sizing on ${home.path}`, async ({
      page,
    }) => {
      await mockAnswerDashboardApi(page);
      await page.setViewportSize({ width: 390, height: 820 });
      ...
      if (home.testId === "shared-home-empty-state") {
        ...
        subtitleFontSize = await subtitle.evaluate((el) =>
          Number.parseFloat(getComputedStyle(el).fontSize),
        );
      }
  ...
  test("phone bottom-dock search keeps the command results sheet hidden", async ({
    page,
  }) => {
    await page.setViewportSize({ width: 390, height: 820 });
    await gotoLauncher(page, "/services?q=13YARN&focus=1&run=1");
    ...
    await expect(
      page.getByRole("listbox", { name: "Services search suggestions" }),
    ).toHaveCount(0);
    await expectNoPageHorizontalOverflow(page);
  });

  test("tablet mode homes keep the shared search in the hero, not the bottom dock", async ({
    page,
  }) => {
    ...
    for (const home of ["/services", "/forms", "/differentials", "/tools"]) {
      ...
      await expect(page.locator("form.answer-footer-search-dock")).toHaveCount(
        0,
      );
  ...
  test("desktop answer footer opens the command surface above the pill", async ({
    page,
  }) => {
    await page.setViewportSize({ width: 1280, height: 900 });
    await mockAnswerDashboardApi(page);
  ...
  for (const viewport of [
    ...
    for (const home of [
      ...
      test(`mode home search is centered at ${viewport.name} width on ${home.path}`, async ({
        page,
      }) => {
        await mockAnswerDashboardApi(page);
        await page.setViewportSize({
        ...
        ).toBeLessThanOrEqual(searchBox?.y ?? 0);

        const metrics = await globalSearchComposerMetrics(page, home.testId);
        expect(metrics, `${home.path} at ${viewport.name}`).not.toBeNull();
        expect(metrics?.position).not.toBe("fixed");
        expect(metrics?.pillClassName).toContain("answer-footer-search-pill");
        expect(metrics?.formWidth ?? 0).toBeLessThanOrEqual(
          viewport.width - 16,
        );
        expect(metrics?.homeLeft).not.toBeNull();
        expect(metrics?.homeRight).not.toBeNull();
        expect(metrics?.homeCenterX).not.toBeNull();
        expect(metrics?.formLeft ?? 0).toBeGreaterThanOrEqual(
          (metrics?.homeLeft ?? 0) - 1,
        );
        expect(metrics?.formRight ?? 0).toBeLessThanOrEqual(
          (metrics?.homeRight ?? viewport.width) + 1,
        );
        expect(
          Math.abs((metrics?.formCenterX ?? 0) - (metrics?.homeCenterX ?? 0)),
        ).toBeLessThanOrEqual(24);
  ...
  for (const viewport of [
    ...
    for (const route of [
      ...
      test(`search route keeps the correct composer at ${viewport.name} width on ${route.path}`, async ({
        page,
      }) => {
        await page.setViewportSize({
          width: viewport.width,
        ...
        await expect(
          page.getByRole("button", { name: route.modeButton }),
        ).toBeVisible({ timeout: 20_000 });
        await expect(
          visibleGlobalSearchInput(page),
        ...
        if (route.ribbonQuery) {
          ...
          await expect(ribbon, `${route.path} at ${viewport.name}`).toBeVisible(
            { timeout: 20_000 },
          );
          await expect(
            ribbon.getByRole("heading", { name: route.ribbonQuery }),
          ).toBeVisible();
        ...
        if (viewport.width < 640) {
          expect(metrics?.position).toBe("fixed");
          expect(metrics?.formCenterY ?? 0).toBeGreaterThan(
          ...
          if (route.compactBottomSearch) {
            ...
            expect(metrics?.formBottom ?? 0).toBeGreaterThanOrEqual(
              viewport.height - 2,
            );
          }
        } else {
          ...
          expect(metrics?.formCenterY ?? viewport.height).toBeLessThan(
  ...
  test("mode home deep links preserve focus=1 on initial load", async ({
    page,
  }) => {
    await page.setViewportSize({ width: 1280, height: 900 });
    ...
    await expect(
      page.getByTestId("services-home").getByTestId("global-search-input"),
    ).toBeVisible();
    await expect(
      page.getByTestId("services-home").getByTestId("global-search-input"),
    ).toBeFocused();
    ...
    await expect(
      visibleByTestId(page, "forms-home").getByTestId("global-search-input"),
    ).toBeVisible();
    await expect(
      visibleByTestId(page, "forms-home").getByTestId("global-search-input"),
    ).toBeFocused();
  ...
  test("services mode shows source-backed records in search results", async ({
    page,
  }) => {
    await page.setViewportSize({ width: 1280, height: 900 });
    await gotoLauncher(page, "/services?q=13YARN&focus=1&run=1");
    ...
    await expect(
      page.locator('input[placeholder="Search services..."]:visible').first(),
    ).toHaveValue("13YARN");
    await expect(page.getByTestId("service-search-results")).toBeVisible();
    await expect(
      page.getByTestId("service-search-result-13yarn"),
    ).toContainText("13YARN");
    await expect(
      page
        .getByTestId("service-search-result-13yarn")
        .getByLabel("Review referral for 13YARN"),
    ).toHaveAttribute("href", "/services/13yarn");
    await expectNoPageHorizontalOverflow(page);
  });

  test("services query param fallback ignores whitespace-only q values", async ({
    page,
  }) => {
    await page.setViewportSize({ width: 1280, height: 900 });
  ...
  test("services results keep browse navigation without a walkthrough", async ({
    page,
  }) => {
    await page.setViewportSize({ width: 1280, height: 900 });
    await gotoLauncher(page, "/services?q=13YARN&focus=1&run=1");
    ...
    await expect(
      page.getByRole("heading", { level: 1, name: "13YARN" }),
    ).toBeVisible();
    await expect(page.getByLabel("Referral workflow")).toHaveCount(0);
    await expect(
      page.getByRole("navigation", { name: "Service groups" }),
    ).toBeVisible();
  ...
  test("forms mode shows registry-backed form records without unsupported pathway claims", async ({
    page,
  }) => {
    await page.setViewportSize({ width: 1280, height: 900 });
    await mockAnswerDashboardApi(page);
    ...
    await expect(page.getByTestId("form-search-results")).toContainText(
      "Best matches",
    );
    await expect(
      page.getByTestId("form-search-result-transport-crisis-form"),
    ).toContainText("Transport order");
    await expect(
      page.getByTestId("form-search-result-extension-transport-order"),
    ).toContainText("Extension of transport order");
    await expect(
      page.getByTestId("form-search-result-detention-examination-movement"),
    ).toContainText("Detention order");
    await expect(
      page.getByTestId("form-search-result-transfer-order"),
    ).toContainText("Transfer order");
    await expect(
      page
    ...
    await expect(
      page.getByRole("navigation", { name: "Forms search sections" }),
    ).toHaveCount(0);
    await expect(page.getByRole("button", { name: "Refine" })).toHaveCount(0);
    await expect(page.getByTestId("form-search-results")).not.toContainText(
      /pathway/i,
    );
    await expect(
      page.getByText(
        /Evidence 278|Pathways 12|Tasks 8|Source verified|Aligned to MHA 2014/,
      ),
    ).toHaveCount(0);
    await expect(
      page.getByText(/PSOLIS Transport|View full pathway/),
    ).toHaveCount(0);
    await expect(page.getByTestId("service-search-results")).toHaveCount(0);
    await expectNoPageHorizontalOverflow(page);
  });

  test("result sorting persists in the URL and restores through browser history", async ({
    page,
  }) => {
    await page.setViewportSize({ width: 1280, height: 900 });
    await mockAnswerDashboardApi(page);
    ...
    const visibleSort = page.locator(
      '[role="group"][aria-label="Sort results"]:visible',
    );
    const sortOption = (name: string) =>
      visibleSort.getByRole("button", { name });
    const expectedAlphaFirstTestId = `form-search-result-${
      sortResultItems(
        rankFormRecords(formRecords, "transport forms"),
        "alpha",
        (match) => match.service.title,
      )[0]?.service.slug
    }`;
    await expect(
      results.locator('article[data-testid^="form-search-result-"]').first(),
    ).toHaveAttribute(
      "data-testid",
    ...
    await expect(
      results.locator('article[data-testid^="form-search-result-"]').first(),
    ).toHaveAttribute("data-testid", expectedAlphaFirstTestId);

    await page.goBack();
    await expect(sortOption("Relevance")).toHaveAttribute(
      "aria-pressed",
      "true",
    );
    await expect(
  ...
  test("form detail pages keep the shared forms search wired to form results", async ({
    page,
  }) => {
    await page.setViewportSize({ width: 1280, height: 900 });
    await mockAnswerDashboardApi(page);
    ...
    const formDetail = await expectSingleSettledOwner(
      page.getByTestId("form-detail-page"),
      {
    ...
    await expect(
      formDetail.getByRole("heading", { level: 1, name: "Transport order" }),
    ).toBeVisible();
    await expect(page.getByTestId("form-search-results")).toHaveCount(0);
    const formsSearchInput = page
      .locator('input[placeholder="Search forms..."]:visible')
      .first();
    await expect(formsSearchInput).toBeVisible();

    await expect(page.getByText("Loading your forms registry...")).toBeHidden({
      timeout: 30_000,
    });
    const formsSearchButton = page.getByRole("button", {
      name: "Search forms",
    });
    await formsSearchInput.fill("transport forms");
    await expect(formsSearchButton).toBeEnabled();
    await waitForReactEventHandler(
      formsSearchButton.locator("xpath=ancestor::form[1]"),
      "onSubmit",
    );
    await formsSearchButton.click();
    await expect(page).toHaveURL(/\/forms\?.*\bq=transport(?:\+|%20)forms\b/, {
      timeout: 20_000,
    });
    await expect(page.getByTestId("form-search-results")).toBeVisible({
      timeout: 20_000,
  ...
  test("form detail mobile renders decision context after the form content", async ({
    page,
  }) => {
    await page.setViewportSize({ width: 390, height: 844 });
    await mockAnswerDashboardApi(page);
    ...
    await expect(
      formDetail.getByTestId("form-decision-context-mobile"),
    ).toBeVisible();
    await expect(
      page.locator('[data-testid="global-search-input"]:visible'),
    ).toHaveCount(1);
  ...
  test("forms search mockup is usable without horizontal overflow on mobile", async ({
    page,
  }) => {
    await page.setViewportSize({ width: 390, height: 844 });
    await mockAnswerDashboardApi(page);
    ...
    await expect(
      page.getByTestId("form-search-mobile-result-transport-crisis-form"),
    ).toContainText("Transport order");
    ...
    await expect(
      page.getByTestId("form-search-mobile-results"),
    ).not.toContainText(/related pathway|view full pathway/i);
    await expect(
      page.getByText(/PSOLIS Transport|View full pathway|Source verified/),
    ).toHaveCount(0);
    await expect(visibleGlobalSearchInput(page)).toHaveValue("transport");
    await expectNoPageHorizontalOverflow(page);
  });

  test("phone bottom search dock stays edge-to-edge with safe-area padding inside the form", async ({
    page,
  }) => {
    ...
    for (const route of [
      {
        path: "/forms?q=transport&run=1",
        resultsTestId: "form-search-mobile-results",
      },
      {
        path: "/differentials?q=acute+confusion&run=1",
        resultsTestId: "differentials-search-results",
      },
    ] as const) {
      await gotoLauncher(page, route.path);
      ...
      await expect(dock, route.path).not.toHaveAttribute(
        "data-scroll-hidden",
        "true",
      );
      ...
      expect(
        Math.abs(geometry.formBottom - geometry.viewportHeight),
        route.path,
      ).toBeLessThanOrEqual(1);
      expect(geometry.paddingBottom, route.path).toBeGreaterThanOrEqual(
        safeAreaBottom - 1,
      );
  ...
  test("phone bottom search dock hides while scrolling down on search results", async ({
    page,
  }) => {
    ...
    await expect
      .poll(async () => readMobileComposerReservePx(main))
      .toBeGreaterThan(112);
    ...
    await appendPrimaryScrollSpacer(page, {
      heightPx: 2000,
      testId: "test-scroll-spacer",
    });
    await expect
      .poll(async () => (await readPrimaryScrollGeometry(page)).owner)
      .toBe("document");
    ...
    expect(hiddenMainGeometry.top).toBeLessThan(
      hiddenMainGeometry.viewportHeight,
    );
    expect(hiddenMainGeometry.bottom).toBeGreaterThanOrEqual(
      hiddenMainGeometry.viewportHeight - 1,
    );
  ...
  test("tablet and desktop forms results keep non-phone bottom clearance", async ({
    page,
  }) => {
    ...
    for (const viewport of [
      ...
      if ((await dock.count()) > 0 && (await dock.first().isVisible())) {
        await expect(dock.first()).not.toHaveAttribute(
          "data-scroll-hidden",
          "true",
        );
      }
  ...
  test("mode toggle keeps the shared home hero geometry when the mode changes", async ({
    page,
  }) => {
    await page.setViewportSize({ width: 1280, height: 900 });
    await gotoLauncher(page, "/?mode=answer");
    ...
    const menu = await openAppModeMenu(page, "Answer");
    await expect(
      menu.getByRole("menuitemradio", { name: /^Services\b/ }),
    ).toBeVisible();
    ...
    const heroSearch = visibleByTestId(
      page,
      "shared-home-empty-state",
    ).getByTestId("global-search-input");
    await expect(heroSearch).toBeVisible();
    ...
    const searchBox = await heroSearch.boundingBox();
    const headingBox = await page
      .getByRole("heading", { level: 2, name: "Clinical Forms" })
      .boundingBox();
    expect(searchBox).not.toBeNull();
    expect(headingBox).not.toBeNull();
    expect((headingBox?.y ?? 0) + (headingBox?.height ?? 0)).toBeLessThan(
    ...
    expect((searchBox?.y ?? 0) + (searchBox?.height ?? 0) / 2).toBeLessThan(
      900 * 0.62,
    );
    await expectNoPageHorizontalOverflow(page);
  });
  ...
  test("the differentials home renders inside the dashboard when opened directly", async ({
    page,
  }) => {
    await page.setViewportSize({ width: 1280, height: 900 });
    ...
    await expect(
      page.getByRole("button", { name: "Mode Differentials" }),
    ).toBeVisible();
    await expect(page.getByTestId("differentials-home")).toBeVisible();
    await expect(
      page.getByRole("heading", { level: 1, name: "Differentials" }),
    ).toBeVisible();
    await expect(
      page
        .locator('input[placeholder="Ask or search a presentation"]:visible')
    ...
    await expect(
      page.getByRole("button", { name: "Search presentations" }),
    ).toBeVisible();
    await expect(
      page.getByRole("button", { name: "Compare differentials" }),
    ).toBeVisible();
    await expect(
      page
        .getByRole("region", { name: "Differential actions" })
        .getByRole("button"),
    ).toHaveCount(2);
    await expect(
      page
        .getByTestId("differentials-home")
        .getByRole("region", { name: /^(Recent work|Library matches)$/ }),
    ).toBeVisible();
    await expect(page.getByTestId("global-search-input")).toHaveCount(1);
    const differentialsHomeSearch = page
      .getByTestId("differentials-home")
      .getByTestId("global-search-input");
    ...
    await expect(async () => {
      const differentialsSearchBox =
        await differentialsHomeSearch.boundingBox();
      ...
      expect(
        (differentialsHeadingBox?.y ?? 0) +
          (differentialsHeadingBox?.height ?? 0),
      ).toBeLessThan(differentialsSearchBox?.y ?? 0);
      expect(
        (differentialsSearchBox?.y ?? 0) +
          (differentialsSearchBox?.height ?? 0) / 2,
      ).toBeLessThan(900 * 0.62);
    }).toPass({ timeout: 10_000 });
    await expectNoPageHorizontalOverflow(page);
  });

  test("differentials global search posts the standalone differentials mode", async ({
    page,
  }) => {
    await page.setViewportSize({ width: 1280, height: 900 });
    ...
    await page.route(/\/api\/search(?:\?.*)?$/, async (route) => {
      ...
      await route.fulfill({
        json: {
          results: [
            {
              ...
              content:
                "Acute confusion with inattention should prioritise delirium and other urgent causes.",
              image_ids: [],
              similarity: 0.91,
              hybrid_score: 0.93,
              images: [],
          ...
          smartPanel: {},
          telemetry: {
            query_class: "differential_compare",
    ...
    await expect(
      visibleByTestId(page, "differentials-search-results"),
    ).toBeVisible();
    await expect(
      page.getByRole("heading", { name: "Differential matches" }),
    ).toBeVisible();
    await expect(
      page
        .getByRole("link", {
          name: "Delirium / Acute Confusion / Encephalopathy",
        })
        .first(),
    ).toBeVisible();
  });

  test("differentials evidence-backed search badges stay single-line on narrow viewport", async ({
    page,
  }) => {
    ...
    const evidenceBackedResults = visibleByTestId(
      page,
      "differentials-search-results",
    );
    await expect(evidenceBackedResults).toBeVisible();
    await expect(
      evidenceBackedResults.getByRole("region", { name: "Source status" }),
    ).toContainText("1 indexed source match");
    ...
    await expect(
      typeGroup.getByRole("radio", { name: /^Diagnoses/ }),
    ).toBeChecked();
    ...
    const railMetrics = await utilities.evaluate((element) => ({
      ...
      lastChildIsPageFilter:
        ...
        "search-query-ribbon-mobile-controls",
    }));
    ...
    // then visibility.
    const phoneSort = utilities.getByRole("group", {
      name: "Sort results",
      includeHidden: true,
    });
    await expect(phoneSort).toHaveCount(1);
    await expect(phoneSort).toBeHidden();
    const filterHeight = await pageFilters.evaluate(
    ...
    expect(badgeMetrics.scrollHeight).toBeLessThanOrEqual(
      badgeMetrics.height + 1,
    );
    await expectNoPageHorizontalOverflow(page);
  });

  test("differentials search badges stay single-line on narrow viewport", async ({
    page,
  }) => {
    await page.setViewportSize({ width: 375, height: 812 });
    ...
    await expect(
      typeGroup.getByRole("radio", { name: /^Presentations/ }),
    ).toBeChecked();
    await typeGroup.getByRole("radio", { name: /^All/ }).click();
    await page.getByTestId("differential-filter-panel-done").click();
    await expect(typeGroup).toBeHidden();
    ...
    await expect
      ...
      .toBe("document");
    await expect
      .poll(async () => (await readPrimaryScrollGeometry(page)).scrollTop)
      .toBe(0);
    ...
    expect(foldLayout!.bestTop).toBeGreaterThanOrEqual(
      foldLayout!.headerBottom - 2,
    );
    ...
    await expect(
      mobileCards.first().getByTestId("differential-mobile-result-rank"),
    ).toHaveText("1");
    const ranks = await mobileCards
      .getByTestId("differential-mobile-result-rank")
      .allTextContents();
    ...
    const uncheckedSelection = mobileCards
      .getByRole("checkbox", { name: /^Add .+ to comparison$/ })
      .first();
    ...
    const checkedName = uncheckedName!
      .replace(/^Add /, "Remove ")
      .replace(/ to comparison$/, " from comparison");
    const selectionTarget = uncheckedSelection.locator("xpath=..");
    await expectMinTouchTarget(selectionTarget, 48);
    const visibleSelectionBox = await selectionTarget
      .getByTestId("differential-selection-box")
      .evaluate((element) => {
        const rect = element.getBoundingClientRect();
    ...
    const checkedSelection = mobileCards.getByRole("checkbox", {
      name: checkedName,
      exact: true,
    });
    ...
    const titleBadgeLayout = await mobileCards.first().evaluate((card) => {
      const title =
        card.querySelector("a span.line-clamp-2") ?? card.querySelector("a");
    ...
    expect(titleBadgeLayout!.badgeTop).toBeGreaterThanOrEqual(
      titleBadgeLayout!.titleBottom - 1,
    );
  ...
  test("mobile differential compare dock hides on scroll down and stays tappable when revealed", async ({
    page,
  }) => {
    await page.emulateMedia({ reducedMotion: "no-preference" });
    await mockAnswerDashboardApi(page);
    ...
    const input = page.locator(
      'input[placeholder="Ask or search a presentation"]:visible',
    );
    const submit = page.locator(
      'button[aria-label="Search differential presentations"]:visible',
    );
    ...
    const compareAction = page.getByTestId(
      "differentials-compare-selected-mobile",
    );
    const dock = page.locator("form.answer-footer-search-dock");
    ...
    await expect(
      scrollport.getByRole("region", { name: "Source status" }),
    ).toContainText("No indexed source matches");
    await expect(
      page.locator("#differentials-mobile-compare-addon-slot"),
    ).toHaveCount(1);
    ...
    await expect(dock).not.toHaveAttribute("data-scroll-hidden", "true");
    await expect
      .poll(async () => readMobileComposerReservePx(mainContent))
      .toBeGreaterThan(180);
      ...
      return {
        ...
        reservePx: pad
          ...
          : style
            ...
            : null,
        reserve:
          style?.getPropertyValue("--mobile-composer-reserve").trim() ?? null,
    ...
    expect(
      clearance.lastBottom ?? Number.POSITIVE_INFINITY,
    ).toBeLessThanOrEqual(clearance.dockTop ?? 0);
    ...
    expect(revealedGeometry.top).toBeGreaterThanOrEqual(
      revealedGeometry.dockTop!,
    );
    expect(revealedGeometry.bottom).toBeLessThanOrEqual(
      revealedGeometry.dockBottom!,
    );
    ...
    expect(
      clearance.lastBottom ?? Number.POSITIVE_INFINITY,
    ).toBeLessThanOrEqual(revealedGeometry.top);

    const reservePx = await readMobileComposerReservePx(mainContent);
    const dockHeight = await dock.evaluate(
      (element) => element.getBoundingClientRect().height,
    );
  ...
  test("diagnosis detail actions stay tappable and tabs stay single-line", async ({
    page,
  }) => {
    ...
    await expectMinTouchTarget(
      detailPage.getByRole("button", { name: "Save diagnosis" }),
    );
    ...
    await expectNoPageHorizontalOverflow(page);
    await expect(
      detailPage.getByRole("tab", { name: "Overview" }),
    ).toBeHidden();
  ...
  test("differentials compare queue launches presentation comparison", async ({
    page,
  }) => {
    ...
    await gotoLauncher(
      page,
      "/differentials/compare?ids=wernicke-encephalopathy&q=Pain",
    );
    await expect(page).toHaveURL(/\/differentials\/compare/);
    ...
    await expect(
      page.getByRole("heading", { level: 1, name: "1 diagnosis selected" }),
    ).toBeVisible();
    await expect(
      queue.getByRole("link", { name: "Wernicke encephalopathy", exact: true }),
    ).toBeVisible();
    await expect(
      page.getByTestId("differential-compare-edit-selection"),
    ).toHaveAttribute("href", /\/differentials\?.*ids=wernicke-encephalopathy/);
    ...
    await expect(
      page.getByRole("heading", { level: 1, name: workflow.title }),
    ).toBeVisible();
    await expect(
      page
        .getByRole("heading", {
          name: `Selected differentials (1 of ${workflow.totalCount})`,
        })
        .first(),
    ).toBeVisible();
    await expect(
      page
        .locator("span:visible", {
          hasText: `+${workflow.totalCount - 1} not selected`,
        })
        .first(),
    ).toBeVisible();
    await expect(
      page.getByRole("heading", { name: "Safety snapshot" }).first(),
    ).toBeVisible();
    await expect(page.getByText("Service details")).toHaveCount(0);
    await expect(page.getByText("Transport order")).toHaveCount(0);
    await expect(
      page
        .getByLabel("Differential review sidebar")
        .getByText("Local content only")
        .first(),
    ).toBeVisible();
    await expect(
      page.getByRole("button", { name: "Copy after review" }),
    ).toBeVisible();
    await expect(
      page.getByRole("button", { name: "Edit columns" }),
    ).toBeDisabled();
    await expect(page.getByTestId("global-search-input")).toHaveCount(0);

    const tableScrolls = await page
      .getByTestId("differential-comparison-scroll")
      .evaluate((element) => {
    ...
    await gotoLauncher(
      page,
      "/differentials/compare?ids=wernicke-encephalopathy",
    );
    await expect(page.getByTestId("differential-compare-queue")).toBeVisible({
      timeout: 30_000,
    });
    await page.getByTestId("differential-compare-open").click();
    await expect(page).toHaveURL(
    ...
    await expect(
      page.getByRole("link", { name: "Back to differentials" }),
    ).toBeVisible();
    await expect(
      page.getByRole("heading", { level: 1, name: workflow.title }),
    ).toBeVisible();
    ...
    await expect(
      mobileComparison.getByRole("button", {
        name: "Column filters unavailable",
      }),
    ).toBeDisabled();
    await expect(
      mobileComparison
        .getByText("Wernicke encephalopathy", { exact: true })
        .first(),
    ).toBeVisible();
    const languageControl = page.getByRole("button", {
      name: "Language and region settings (coming soon)",
    });
    await expect(languageControl).toBeVisible();
    await expect(languageControl).toHaveAttribute("aria-disabled", "true");
    await expect(
      page.getByRole("button", { name: "Start a new comparison" }),
    ).toBeVisible();
    await expect(page.getByTestId("global-search-input")).toHaveCount(0);
    await expect(page.getByText("Service details")).toHaveCount(0);
    await expect(page.getByText("Transport order")).toHaveCount(0);
  ...
  test("tools mode opens tool details before navigation on mobile", async ({
    page,
  }) => {
    await page.setViewportSize({ width: 390, height: 820 });
    await gotoLauncher(page, "/tools");
    ...
    const detailsButton = toolsHub.getByRole("button", {
      name: "View details for Medication Prescribing",
    });
    await expect(detailsButton).toHaveAttribute("aria-haspopup", "dialog");
    await detailsButton.click();
    await expect(
      page.getByRole("dialog", { name: "Medication Prescribing" }),
    ).toBeVisible();
    await expectNoPageHorizontalOverflow(page);
  });
});
...
test.describe("Clinical KB service detail page", () => {
  ...
  for (const viewport of [
    ...
    test(`13YARN service detail is usable at ${viewport.name}`, async ({
      page,
    }) => {
      await mockAnswerDashboardApi(page);
      await page.setViewportSize({
      ...
      const servicePage = page
        .locator('[data-testid="service-detail-page"]:visible')
        .last();
      await expect(servicePage).toBeVisible();
      await expect(
        servicePage.getByRole("heading", { level: 1, name: "13YARN" }),
      ).toBeVisible();
      ...
      await expect(
        actions.getByRole("button", { name: "Save service" }),
      ).toBeVisible();
      await expect(actions.getByRole("link", { name: "Call" })).toHaveAttribute(
        "href",
        "tel:139276",
      );
      await expect(
        actions.getByRole("button", { name: "Use in navigator" }),
      ).toBeVisible();
      await expect(
        servicePage.getByRole("link", { name: "Call 13 92 76" }),
      ).toHaveAttribute("href", "tel:139276");
      await expect(
        servicePage.getByRole("heading", { name: "Priority facts" }),
      ).toBeVisible();
      await expect(
        servicePage.getByRole("button", { name: /copy/i }),
      ).toHaveCount(0);
      await page.keyboard.press("Escape");
      ...
      await expect(
        servicePage.locator('[data-testid="global-search-input"]'),
      ).toHaveCount(0);
      await expect(
        servicePage.getByPlaceholder(/Search services/i),
      ).toHaveCount(0);
      await expectNoPageHorizontalOverflow(page);
    });
  }

  test("service referral action remains keyboard reachable in reduced motion and forced colors", async ({
    page,
  }) => {
    await mockAnswerDashboardApi(page);
    await page.setViewportSize({ width: 390, height: 820 });
    ...
    const servicePage = page
      .locator('[data-testid="service-detail-page"]:visible')
      .last();
    const referralAction = servicePage.getByRole("link", {
      name: "Call 13 92 76",
    });
  ...
  test("long mobile service details clear the bottom search dock at the scroll endpoint", async ({
    page,
  }) => {
    await mockAnswerDashboardApi(page);
    await page.setViewportSize({ width: 390, height: 820 });
    await gotoLauncher(
      page,
      "/services/city-east-community-mental-health-service",
    );
    ...
    const servicePage = page
      .getByTestId("mobile-composer-reserve-pad")
      .getByTestId("service-detail-page");
    const footer = servicePage.getByText(
      "Information accuracy may vary. Confirm locally before use.",
    );
    const mainContent = page.locator("#main-content");
    const dock = page
      .locator("form.answer-footer-search-dock, form.answer-footer-search-edge")
      .first();
    ...
    expect(
      clearance,
      JSON.stringify({ clearance, scrollGeometry }),
    ).not.toBeNull();
    expect(
      clearance!.reservePx,
      JSON.stringify({ clearance, scrollGeometry }),
    ...
    expect(
      clearance!.footerBottom,
      JSON.stringify({ clearance, scrollGeometry }),
    ).toBeLessThanOrEqual(clearance!.dockTop - 8);
  });

  test("service navigator action uses the shared global search route", async ({
    page,
  }) => {
    await mockAnswerDashboardApi(page);
    await page.setViewportSize({ width: 1280, height: 900 });
    ...
    await page.getByTestId("service-actions-trigger").click();
    await page
      .getByTestId("service-actions-sheet")
      .getByRole("button", { name: "Use in navigator" })
      .click();
  ...
  test("service actions keep a distinct verification source beside the contact route", async ({
    page,
  }) => {
    await mockAnswerDashboardApi(page);
    await page.setViewportSize({ width: 390, height: 820 });
    ...
    await expect(
      actions.getByRole("link", { name: "Open source" }),
    ).toHaveAttribute("href", /^https?:\/\//);
    await expect(
      page.getByRole("link", { name: "View service source" }),
    ).toHaveAttribute("href", /^https?:\/\//);
  });

  test("service detail actions save and back from direct entry", async ({
    page,
  }) => {
    await mockAnswerDashboardApi(page);
    await page.setViewportSize({ width: 1280, height: 900 });
    ...
    await actionsTrigger.click();
    await expect(
      actions.getByRole("button", { name: "Remove saved service" }),
    ).toBeVisible();
    await expect(
      actions.getByRole("button", { name: "Copy contact" }),
    ).toHaveCount(0);
    await page.keyboard.press("Escape");
...
test.describe("Responsive layout guards", () => {
  ...
  for (const route of mockupRoutes) {
    test(`${route.name} never overflows horizontally across sizes @mockup`, async ({
      page,
    }) => {
      await page.setViewportSize({ width: 1280, height: 900 });
      await gotoLauncher(page, route.path);
  ...
  for (const route of modeHomeRoutes) {
    test(`${route.name} mode home never overflows horizontally across sizes`, async ({
      page,
    }) => {
      await page.setViewportSize({ width: 1280, height: 900 });
      await gotoLauncher(page, route.path);
  ...
  test("prescribing mode home centres above the phone composer and balances on tablet", async ({
    page,
  }) => {
    async function verticalWeighting(width: number) {
      ...
      const measure = () =>
        page.evaluate(() => {
          const rect = document
            .querySelector('[data-testid="medication-home"]')
            ?.getBoundingClientRect();
          if (!rect) return null;
          return {
            topGap: rect.top,
      ...
      await expect(async () => {
        ...
        const stable =
          result !== null &&
          next !== null &&
          result.topGap === next.topGap &&
          result.bottomGap === next.bottomGap;
        result = next;
    ...
    expect(balance).toBeLessThan(
      Math.max(tablet?.topGap ?? 0, tablet?.bottomGap ?? 0) * 1.45,
    );
  });

  test("prescribing mobile shortcuts and checks are distinct, actionable, and scrollable", async ({
    page,
  }) => {
    ...
    await expect(home).toContainText(
      "Check renal dosing and contraindications.",
    );
    await expect(home).toContainText(
      "Review opioid-use precautions before prescribing.",
    );
    await expect(home).toContainText(
      "Check maximum dose and titration guidance.",
    );

    const checksRegion = home.getByRole("region", {
      name: "Medication checks",
    });
    const checkButtons = checksRegion.getByRole("button");
    await expect(checkButtons).toHaveCount(4);
    for (const button of await checkButtons.all())
      await expectMinTouchTarget(button);

    const rowMetrics = await checksRegion
      .locator(".answer-suggestion-row-scroll")
      .evaluate((row) => {
    ...
    await gotoLauncher(
      page,
      "/?mode=prescribing&q=acamprosate%20renal%20dose&run=1",
    );
    ...
    await expect(
      queryRibbon.getByRole("heading", { name: "acamprosate renal dose" }),
    ).toBeVisible();
    await expect(queryRibbon.getByRole("status")).toBeVisible();
    const resultFilter = queryRibbon.getByTestId(
      "medication-filter-trigger-phone",
    );
  ...
  test("safety-plan working content stays local until an explicit export", async ({
    page,
  }) => {
    ...
    await expect(page.getByLabel(/Patient \(name or initials\)/i)).toHaveCount(
      0,
    );
    const privacyRegion = page.getByRole("region", {
      name: "Safety plan privacy",
    });
    await expect(privacyRegion).toHaveCount(1);
    await expect(
      privacyRegion.getByText(/kept only in this browser tab/i),
    ).toBeVisible();
    ...
    await expect(
      page
        ...
        .getByText(
          /Copying, printing, or saving a PDF moves the plan outside Clinical KB/i,
        ),
    ).toBeVisible();
    ...
    await page
      .getByLabel("e.g. Not sleeping for a couple of nights")
      .fill("Not sleeping");
    ...
    await expect
      .poll(() =>
        page.evaluate(
          () =>
            (window as typeof window & { __copiedPlan?: string }).__copiedPlan,
        ),
      )
    ...
    await expect
      .poll(() =>
        page.evaluate(
          () =>
            (window as typeof window & { __printCalled?: boolean })
              .__printCalled,
        ),
      )
      .toBe(true);
  ...
  test("differentials recent work remains touch-sized inside its mobile scroll row", async ({
    page,
  }) => {
    await page.setViewportSize({ width: 320, height: 760 });
    await mockAnswerDashboardApi(page);
    ...
    const recentWork = page
      .getByTestId("differentials-home-template")
      .getByRole("region", { name: "Recent work" });
    await expect(recentWork).toBeVisible();
    const recentButtons = recentWork
      .locator(".answer-suggestion-row-scroll")
      .getByRole("button");
    expect(await recentButtons.count()).toBeGreaterThan(1);
    for (const button of await recentButtons.all())
      await expectMinTouchTarget(button);

    const rowMetrics = await recentWork
      .locator(".answer-suggestion-row-scroll")
      .evaluate((row) => {
        const style = getComputedStyle(row);
        return {
