...
describe("the out-of-area ledger", () => {
   ...
   * difference in the result. Deleting the `state === "departed"` guard in `outOfAreaLedger` makes
  ...
  /**
   * The occupancy check is an ALLOWLIST, not a `state !== "departed"` denylist, and this is the test
...
describe("the out-of-area ledger over the seeded wards", () => {
  ...
  it("counts occupied out-of-area beds, and every entry is genuinely one", () => {
    for (const entry of ledger.entries) {
      expect(entry.admission.state).not.toBe("departed");
