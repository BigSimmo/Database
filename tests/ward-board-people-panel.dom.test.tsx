...
describe("a person who is away at an emergency department", () => {
  ...
  it("changes no bed figure ON THE RENDERED BOARD — the ward is holding the bed", () => {
    /*
     ...
     * coordinator would then be offered a bed with somebody in it.
     *
     * ⚠️ REWRITTEN 2026-09-01, BECAUSE THE VERSION THAT STOOD HERE COULD NOT FAIL — and its own
     * comment said it could ("if anything on this board ever starts reading the away field for
     * capacity, this goes red"). The decisive assertion compared
     * `admissionsForUnit(wardAdmissions, unitId).filter(bedIsOccupied).length` against
     * `awayAdmissions.filter(...).length`: BOTH sides read the imported seed, and neither read the
     * render. Its only render-touching assertion was `occupantsDrawn > 0`, a floor the ward's
     * seventeen ordinary occupants hold up on their own. Adding `.filter((a) =>
     * a.awayAtEmergencyDepartmentSince === null)` to `buildTiles` and `buildOccupants` — the exact
     * defect the comment named — left it green.
     *
     * So every number below is READ OFF THE RENDERED BOARD and compared to the seed. The seed
     * supplies the expectation; the screen is the thing under test. A capacity claim a test cannot
     * see is a capacity claim nothing checks.
     */
    const unitId = awayAdmissions[0].unitId;
    const unit = ALL_UNITS.find((candidate) => candidate.id === unitId);
    if (!unit) throw new Error(`the seed has no unit ${unitId}`);
    const awayHere = awayAdmissions.filter((admission) => admission.unitId === unitId);
    const occupiedInSeed = admissionsForUnit(wardAdmissions, unitId).filter(bedIsOccupied).length;
    expect(awayHere.length, "this unit has nobody away at an ED; the assertions below are vacuous").toBeGreaterThan(0);

    const view = renderWardBoard(unitId);

    /*
     * 1. THE SENTENCE UNDER THE GRID — "18 of this ward's 20 beds are taken." The figure a ward
     *    reads without counting tiles, and the one that moves first if an away person stops being
     *    counted as an occupant.
     */
    const occupancySentence = (view.getByTestId("ward-board-people-count").textContent ?? "").trim();
    const takenOnScreen = Number(/(\d+)\s+of this ward/u.exec(occupancySentence)?.[1] ?? Number.NaN);
    expect(
      takenOnScreen,
      `the board says "${occupancySentence}" — the seed puts ${occupiedInSeed} people in beds on this ward, ` +
        `${awayHere.length} of them away at an emergency department and still holding their beds`,
    ).toBe(occupiedInSeed);
    expect(
      Number(/of this ward's (\d+) bed/u.exec(occupancySentence)?.[1] ?? Number.NaN),
      `the sentence names a bed count this ward does not have: "${occupancySentence}"`,
    ).toBe(unit.beds);

    /*
     * 2. THE GRID ITSELF. A bed is taken whether the person in it has arrived (`occupied`) or the
     *    ward has already given it away to somebody travelling (`waiting`); neither is a bed
     *    anybody can be offered.
     */
    const kinds = view.getAllByTestId(/^ward-board-bed-\d+$/u).map((tile) => tile.getAttribute("data-bed-kind"));
    const drawnTaken = kinds.filter((kind) => kind === "occupied" || kind === "waiting").length;
    expect(
      drawnTaken,
      `the grid draws ${drawnTaken} beds as taken but ${occupiedInSeed} people are in beds here — ` +
        `${awayHere.length} of them away at an emergency department`,
    ).toBe(occupiedInSeed);
    ...
    expect(
      ...
      `the grid offers ${offerableDrawn} beds on a ward with ${unit.beds} beds, ${occupiedInSeed} of them taken ` +
        `and ${blockedDrawn} out of service — a bed belonging to somebody away at an emergency department ` +
        `has been put back in the pool`,
    ).toBe(unit.beds - occupiedInSeed - blockedDrawn);

    /*
     * 4. THE SAME CLAIM PER PERSON, on the surface a ward scans. The away mark must sit on a tile
     *    the grid still calls occupied; a mark on a freed tile would be the badge without the
     *    property it exists to state.
     */
    const awayTiles = view.getAllByTestId(/^ward-board-bed-\d+-away$/u);
    expect(awayTiles, "no bed tile carries the away marker, so nothing below is checked").toHaveLength(awayHere.length);
    for (const mark of awayTiles) {
      const tile = mark.closest("[data-bed-kind]");
      expect(
        tile?.getAttribute("data-bed-kind"),
        "a bed marked as its occupant being at an emergency department is no longer drawn as occupied",
      ).toBe("occupied");
    }
  });
});
