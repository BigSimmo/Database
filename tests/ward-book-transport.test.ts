...
describe("BOOK_TRANSPORT", () => {
   ...
   */
  it("lets a COMMUNITY team book, and writes the IDENTICAL transport record", () => {
    const { movementId } = heldBedMovement();
    const byCommunity = book({ role: "community" });
    ...
    // The same record, not merely a successful one. The reducer stores `provider` and the escort
    // answer and NOTHING about the caller — no role, no team, no person — so a community booking
    // must be indistinguishable from the ED's. If a role name ever leaks into the transport record,
    // this is the assertion that fails rather than a screen that quietly attributes a job.
    expect(community).toEqual(byEd);
  });
