...
describe("Caring Contacts stays out of the live Tools catalogue", () => {
  it("has no caring-contacts card (workspace is developer-hub only)", () => {
    expect(toolCatalogRecords.map((tool) => tool.id)).not.toContain("caring-contacts");
    expect(toolCatalogRecords.some((tool) => tool.href === "/caring-contacts")).toBe(false);
    for (const session of [
      { authenticated: false, demoMode: false },
      { authenticated: true, demoMode: false },
      { authenticated: true, demoMode: true },
    ] as const) {
      expect(toolCatalogRecordsForSession(session).some((tool) => tool.href === "/caring-contacts")).toBe(false);
    }
  });
});
