...
// setup-codex-cloud.sh is Linux/Cloud-only; WSL Git cannot resolve a Windows
// worktree's gitdir pointer, so these behavioral cases run in Linux CI.
const linuxOnlyIt = process.platform === "win32" ? it.skip : it;
...
describe("Codex Cloud environment contract", () => {
  ...
  linuxOnlyIt("writes managed shell policy behaviorally and preserves unrelated Codex config", () => {
  ...
  linuxOnlyIt("pins connected retrieval mode and rejects unsafe shell-policy configs", () => {
