...
| Chip sizes       | `Chip` `compact` → `--text-2xs` (11px); `standard` (default) → `--text-xs` (12px)                                                         | Canonical (#220): dense surfaces opt into `size="compact"`; default adoption keeps 12px. Do not densify every Chip to chase the globals “small chips” note.         |
