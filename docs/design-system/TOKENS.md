...
| Chip sizes       | `Chip` `compact` → `--text-2xs`; `standard` (default) → `--text-xs`                                                                       | Canonical (#220): dense surfaces opt into `size="compact"`; default adoption keeps `--text-xs`. Do not densify every Chip to chase the globals “small chips” note.  |
