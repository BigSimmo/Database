...
Track A is almost entirely invisible — it stops the _next_ change from breaking something. The
list below is the part a person using the app would notice, so it is the part worth showing the
owner and worth looking at in a browser rather than a diff.

**Already changed** (landed with #1658): the answer now sits inside a bordered, raised card
instead of floating on a transparent background. This is the design system's stated intent —
`answer-card.tsx` notes that "the screen the product is judged on had no surface at all" — but
it is a real change to the most-used screen. **Look at this first.** If the framing reads
wrong, that is a styling decision, not a rebuild.

| #   | Change                                                                                                                                             | Where                       | How visible                                                                                      |
| --- | -------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------- | ------------------------------------------------------------------------------------------------ |
| 1   | **Forms become consistent** — same label weight, same error styling, and a format hint that no longer disappears at the moment the format is wrong | every form surface (B1)     | high — biggest single win                                                                        |
| 2   | **Bare dashes become words** — "Not recorded" / "Unknown" / "Not applicable"                                                                       | 19 sites (B3)               | high, and clinically material: an empty potassium cell and a normal one currently look identical |
| 3   | **Search composer focus ring** — soft glow becomes a crisp outline, matching every other control                                                   | search composer (A2)        | medium, on focus only                                                                            |
| 4   | **Buttons, links, tabs, tooltips, toasts unify** — spacing, focus rings, disabled states                                                           | per surface as adopted (B1) | low individually, clear in aggregate                                                             |
| 5   | **Every component proven in every state** — hover, disabled, 320px, dark, forced colours, print                                                    | all (B4)                    | invisible until something would have broken                                                      |

Items 1–4 are safe to do while the baseline hold stands, because each is small enough to review
by eye on its own PR. Item 5 cannot start until §5 is resolved.
