...
static class extraction cannot reach: a colour that must be a *value* (inline style), chosen at
...
5. **`--container-phone-frame` (390px) duplicated as a JS constant** — `PHONE_WIDTH = 390` defined in
   ...
   token; paired with a real *value* drift where `--radius-phone-frame` (1.85rem) is hardcoded as
...
_Pending — dispatched alongside the ratchet-gaming review; findings appended here once both return._
