...
You need a terminal window open in your copy of the project folder on your computer. On a
Mac, open the Terminal app and type `cd ` (with a space), drag the project folder into the
window, and press Enter. On Windows, open the project folder, right-click an empty space and
...
2. Switch to the version that has the sign-off tool:
...
3. Check the tool works. This only shows what is waiting and changes nothing:

   ```bash
   npm run clinical:review
   ```

   If this says "command not found" or "Cannot find module", the app is not set up on this
   computer yet. Run this once (it takes a few minutes), then try step 3 again:
...
4. Start the walk-through. Replace `<your name>` with your name exactly as you want it shown
   in the app, for example `"Dr J Smith"`. Keep the quote marks.

   ```bash
   npm run clinical:review -- --write --walk --kind form --reviewed-by "<your name>"
   ```
...
5. The tool shows one form at a time, highest consequence first: **3C, 10B, 10E, 11B, 11E,
   6C**, then the rest in catalogue order. For each form it shows everything you are
   vouching for: the purpose, who may make it, when it applies, the clock, what it does and
...
6. Answer the three questions. Type `yes` or `no` and press Enter.
...
7. After three `yes` answers, type the form's code to confirm (for example `3C`) and press
   Enter. That form is saved straight away.

8. Stop whenever you like: type `quit` at any question, or press Ctrl and C together. Every
   form you already confirmed stays saved. Next time, the walk-through starts at the first
...
```bash
npm run clinical:review -- --write --walk --kind section --reviewed-by "<your name>"
```
...
9. Mark the changed files to be saved. For forms:
...
10. Save them with a short note:
...
Then tell Claude "I've pushed my sign-offs", and Claude will take it from there. If the
push step gives an error, tell Claude "commit my sign-offs" instead, and Claude will send
them for you.
...
- **Anything else.** Copy the last line of the message and ask Claude.
