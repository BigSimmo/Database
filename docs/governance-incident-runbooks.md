...
2. Confirm answer caches for the owner are invalidated after the transaction commits.
3. Verify the source remains directly browsable but is excluded from answer generation when outdated, rejected, decommissioned, superseded, or poorly extracted.
4. Re-run source-governance and focused answer tests. Live data repair or migration requires explicit approval.
