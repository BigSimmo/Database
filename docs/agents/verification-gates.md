...
- The post-merge browser matrix uses one Chromium runner and three partitions each for Firefox and WebKit, retaining one worker and zero retries per runner. Its primary path excludes only projects already proved by the production shards; mobile WebKit and standalone PWA still run. This is test partitioning, not selective omission. Hosted duration and runner-minute improvements remain unmeasured until the change runs on GitHub; large test files can still produce uneven shards.
...
- Successful production UI shards retain their JSON reports for seven days so future balancing can use current per-file timings. Do not treat historical zero-duration entries in the shard profile as measurements or automatically remove tests because they have recently passed.
