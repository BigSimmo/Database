...
**Owner's words, 2026-08-29:** _"a screen made simple for transport officers etc rather than
cluttering the main nav."_
...
board, and the honest answer — _they cannot really, it is just the prototype_ — is an explanation
...
- the community mental health team _(links to the per-region record in `ward-teams.ts`)_
...
usual failure is a row outliving its session, not a row outliving its session's _branch_.
