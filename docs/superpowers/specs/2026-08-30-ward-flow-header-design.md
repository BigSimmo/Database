...
1. **Transport has no header.** Plumbing — give it the same one. _(Ward Core.)_
2. **`ward-management-modes.tsx:170` hardcodes `15 Aug 2026 · WA` beside a live clock.** A frozen
   date next to a moving one, and a changeable real-world fact written into a component. _Already
   routed as a defect — not design._
...
catchment data must render as unreviewed (`CM-7`), a diagnosis is labelled _tentative_, the transport
...
⚠️ **No service-wide state.** _"Three patients waiting in ED"_ on every screen is the obvious
...
- **No actions.** _"Nothing a coordinator acts on"_ is a true observation and **not a fault to fix**:
