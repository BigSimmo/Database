...
- **Wave 1 (dispatch now, parallel, disjoint files):** S1c (`rag-claim-support.ts`), S1d
  ...
  `answer-stream-contract.ts`, `rag-row-contracts.ts`), S6 (`eval/docling/`). S5 is merged.
  S5 follow-ups queued as inbox requests: three adversarial divergence pins
  (`cite-mismatched-attribution`, `scope-other-owner-document`, `scope-guessed-chunk-id`) and
  the owner decision on enabling `RAG_TELEMETRY_EXTENDED` in production.
