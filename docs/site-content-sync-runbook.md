...
7. Point 6 governs what a **completed** read may answer with. A read that cannot complete at all is separate: on 2026-09-15 the project's Supabase edge returned HTTP 520 (x20), 521, 522, 525 and 503, which carry a Cloudflare HTML error page rather than a PostgREST body, and `/api/medications` answered 500 for the entire catalogue. `readCanonicalSiteContentRecords` therefore takes an explicit `fallbackToSeedsOnUnavailable` opt-in, **off by default**. Publication, reconciliation, search, the differential surfaces and the registry routes do not set it and keep failing loudly, because a silent fall back would let an operator believe they were looking at live canonical state. Only `/api/medications` and `/api/medications/[slug]` opt in. They answer from the bundled seeds, mark the body `retainedSnapshot: true`, leave `fixture` false so the response is `private, no-store` and cannot be held by a CDN past the outage, and render `RetainedSnapshotNotice` so the reader is told the records are a stored copy and that recent published edits may be missing. An abort still propagates, and the upstream failure text is never logged because it can be a whole HTML page. Pinned by `tests/site-content-retained-snapshot.test.ts`.
