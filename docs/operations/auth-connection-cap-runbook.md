...
any horizontal replica add. See the ordering in `docs/architecture/deployment-architecture.md`
...
effectively **~10 absolute connections** (see `docs/guides/process-hardening.md`). Auth
...
- **Keep single-instance deploys** until this is done — every extra cold app
  ...
  (`docs/architecture/deployment-architecture.md` §2).
...
- `docs/architecture/deployment-architecture.md` §2 (single-instance scale-out plan) and §5
  (why staging must not share the production auth cap).
- `docs/guides/process-hardening.md` (advisor finding, known follow-up debts).
