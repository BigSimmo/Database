...
`docs/architecture/deployment-architecture.md` §5 (why staging is a dedicated project, not a
...
2. **Reconcile and apply the complete reviewed schema chain.** From a checkout linked to the
   staging project, inspect the divergent history and dry-run the same full chain before any apply:
...
Host: **Railway**, same as production (see `docs/architecture/deployment-architecture.md` §2).
...
2. Tenancy isolation: configure the dedicated A/B test accounts and standalone
   ...
   [`staging-tenancy-release-evidence.md`](../evidence/staging-tenancy-release-evidence.md).
