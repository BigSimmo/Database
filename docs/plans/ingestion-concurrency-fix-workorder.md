...
`docs/architecture/ingestion-state-machine.md` (§6 findings, §8 backlog) and
...
(which is known-drifted — see R24e and `docs/operations/database-drift-detection.md`).
...
1. **Never raw-SQL against live.** Every DB change is a committed migration
   ...
   `docs/operations/database-drift-detection.md`. Operator applies with `supabase db push`
   after review.
...
5. **Drift-backlog coordination.** Items touching `ingestion_jobs` indexes
   ...
   backlog in `docs/operations/database-drift-detection.md` (§ "Reconciliation backlog"
...
`docs/operations/database-drift-detection.md` item #8.
