...
- `/caring-contacts/templates` - The governed pathway versions a team holds: lifecycle state, the recorded facts of publication and retirement, and who approved each one — qualified by the record's own provenance, so a synthetic approval never reads as a real one. A governance record viewer; it shows no message wording, because no version holds wording of its own. Source: `src/app/caring-contacts/templates/page.tsx`.
