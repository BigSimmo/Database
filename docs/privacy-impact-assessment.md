...
material shortfalls are **governance/contractual** (APP 8 cross-border) plus the
**hardening** item still open (conditional HMAC); un-redacted answer retention (PIA-3) is now closed —
the durable answer log is dropped at rest by default (short-lived response cache aside), and APP 1/5 notices
are now live (PR #513). None of these are cross-tenant
