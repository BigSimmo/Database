...
| -------------------- | --------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------- |
| `rag_queries`        | No (hash placeholder) | `queryTextForStorage` / `normalizedQueryTextForStorage` ([query-privacy.ts](../src/lib/query-privacy.ts)); centralized write in `insertRagQuery`                                                                                                    | `answer` is null by default and stored only with explicit `RAG_PERSIST_ANSWER_TEXT=true`; `source_chunk_ids` and stable document IDs, but no filename/title telemetry | owner-read, [schema.sql](../supabase/schema.sql)          |
| `rag_query_misses`   | No (hash placeholder) | same helpers; writes in [search route](../src/app/api/search/route.ts) and [interaction route](../src/app/api/search/interaction/route.ts)                                                                                                          | New rows retain clicked document/chunk IDs, top chunk IDs, and ranking facts; caller filenames/titles are ignored and `top_files` is empty                            | owner-read, [schema.sql](../supabase/schema.sql)          |
...
Label create, edit, review, hide, restore, and delete operations write `document_label_change` audit rows after the mutation succeeds. The actor/owner is the authenticated `owner_id`, the label ID is `resource_id`, and the only retained label metadata is the bounded operation plus document UUID. Label text is not copied into the indefinite audit trail.

Document display names are resolved only when an authorized operator runs the query-miss promotion reader. That reader remains compatible with historical `top_files` rows while resolving newer ID-only rows through `document_chunks` and `documents`. Existing historical telemetry is not purged or rewritten by this change; its normal retention schedule remains authoritative. Runtime truncation logs similarly use document IDs, stage, status, and reason rather than filenames or titles.
