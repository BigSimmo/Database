...
1. Preflight: `check:supabase-project`, `supabase:recovery-status`, `reindex:health` (abort if
   `supabase_unavailable` or queue in recovery — per `docs/operations/reindex-runbook.md`).
...
- Run `npx supabase db advisors --linked` and update `docs/operations/supabase-migration-reconciliation.md`.
