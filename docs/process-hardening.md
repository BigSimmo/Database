...
- `npm run verify:full` is the broad offline local gate for cross-module risk: the 39 static/consistency gates (`check:runtime` through `check:calculator-content`; `npm run check:gate-manifest` lists them and pins the count), then lint, typecheck, and unit tests. It is selected, not automatic for every source/config/test edit.
