...
- **Known follow-up debts (not reconciled in this change):**
  - `search_schema_health()` diverged on two lineages — the live copy carries the execution smoke but dropped schema.sql's comprehensive `required_indexes` / signature checks and the `detect_legacy_ivfflat_indexes()` report. Merge into one superset definition (comprehensive checks + execution smoke) via a migration.
  ...
  - Auth server is capped at 10 absolute DB connections (Supabase advisor); switch to percentage-based allocation before scaling instance size.
