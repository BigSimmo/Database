...
| **Figma**                   | Cursor Figma plugin + `.cursor/mcp.json` → `https://mcp.figma.com/mcp` (OAuth)   | Desktop Cursor: capture live UI, read/write frames, Make context, Code Connect                                                               | **Cursor Cloud Agents unsupported** for official Figma MCP today; use Desktop or Codex Figma plugin. Treat Make as exploration; product truth is `docs/design-system/` + gates; keep ≤5 active MCPs |
