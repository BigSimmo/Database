...
| **Context7**                | Codex Desktop: installed Context7 app when callable. Cursor: `.cursor/mcp.json` → `npx -y @upstash/context7-mcp@4.0.2` (+ `context7-plugin`)                                  | Versioned docs for **Tailwind 4, Zod 4, Playwright, Vitest, React 19, `@supabase/supabase-js`** (peers; not exhaustive). Resolve ID → query one topic; local stdio reads `CONTEXT7_API_KEY` from env | Next.js 16 — always use `node_modules/next/dist/docs/` (AGENTS.md). Never send secrets or proprietary source; never commit the API key           |
...
2. **Project MCP (checked-in):** `.cursor/mcp.json` runs pinned local
   `@upstash/context7-mcp@4.0.2` with `env.CONTEXT7_API_KEY: ${env:CONTEXT7_API_KEY}` so the
...
3. **Cursor Cloud Agent Secrets:** inject `CONTEXT7_API_KEY` into the agent shell
   ...
   `npx ctx7`) and project stdio MCP after reload. It does **not** configure a separate
   host-installed Context7 app; authenticate and verify that app in its host. The Codex Desktop
   app successfully resolved `/upstash/context7` and queried docs on 2026-08-13, but callability
   is session-specific. If the host path is unavailable or rate-limited, use
   `npx ctx7 library|docs …` (or the project local MCP after reload).
...
7. Prefer `resolve-library-id` → `query-docs` when the authenticated MCP path is available;
   query one focused topic and use a versioned `/org/project/version` ID when Context7 lists the
   version in use. A user-supplied valid ID can skip resolution. Otherwise use `npx ctx7`.
   Do not send secrets, personal data, or proprietary source in queries, and avoid raw `curl`
   with keys in chat logs unless debugging.
