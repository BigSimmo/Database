...
2. **Add a CI guard against un-scoped owner tables (cheap, high value) — DONE (2026-07-17),
   ...
   **How a declared proof is verified.** Five of the seven proof kinds are checked in the AST rather
   ...
   **and** filters `documents.owner_id`); `owner-pinned-document-id` (the identifier used as this
   ...
   carrying no tenancy column has no ownership relation for a proof to check.

   **Residual gaps, recorded rather than closed.**
   - **The mechanical proofs are order-insensitive.** `scopeFacts` scans the whole enclosing
     scope, so a proof is satisfied by an ownership query that runs **after** the protected read,
     or whose result is discarded without a branch. The "a 404 is returned before this query"
