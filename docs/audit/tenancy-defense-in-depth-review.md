...
2. **Add a CI guard against un-scoped owner tables (cheap, high value) — DONE (2026-07-17),
   ...
   **How a declared proof is verified.** Five of the six proof kinds are checked in the AST rather
   ...
   **and** filters `documents.owner_id`); `owner-pinned-document-id` (the identifier used as this
   ...
   and every use of it is listed below.

   **Residual gaps, recorded rather than closed.**
   - `documents/[id]/table-facts` PATCH writes review metadata back to `fact.source_image_id`
     filtered by image id alone. It is safe — the fact row was read under
     `.eq("document_id", id).eq("owner_id", user.id)` and the preceding read confirmed the image
     carries that same `document_id` — but the write chain itself does not restate the document
     constraint, so it is the narrowest remaining derived-tier write and is declared
     `reviewed-indirect`.
...
| `src/app/api/documents/[id]/table-facts/route.ts` | `document_images`      | `PATCH`                                                   | owned-document-helper (`id`), reviewed-indirect | The read is constrained to `.eq("document_id", id)` where `loadOwnedDocument` proved ownership. The metadata write targets `fact.source_image_id` by image id alone — safe, but the narrowest remaining derived-tier write; see "Residual gaps" above. |
