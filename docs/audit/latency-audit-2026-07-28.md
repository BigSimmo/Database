...
### L3-3 · Catalogue JSON is uncached and traverses the proxy (originally overstated as 3.16 MB)

`src/components/therapy-compass/bindings.tsx:207-211` → `data/use-therapy-data.ts:28,34-40` · gate=`#017`

**Corrected 2026-07-29 — the original "3.16 MB on mount" claim was wrong twice over.** The two payloads are **mutually exclusive**, not additive: `bindings.tsx:206-211` selects `catalogue: "index"` for home/search/pathways and `"full"` only for detail/compare/recommend/artifact, and `use-therapy-data.ts:35` fetches exactly one. So mode-home first paint is **707 KB, not 3.16 MB**. And the index split this finding implied was missing **already ships** — `scripts/build-therapies-index.mjs` emits both a 474 KB server index and the 707 KB browser index from the 2.53 MB source, gated by `check:therapy-data-index` and pinned by a <40 % size ratio in `tests/therapy-compass-mode-wiring.test.ts:83-88`.

The genuine residual is smaller and has two parts: (a) **nothing sets `Cache-Control` for any `/public/*` path** (`next.config.ts:93-127` covers only `/sw.js`, `/offline.html`, `/manifest.webmanifest`), and the service worker does not cache these files either — `public/sw.js:37-51` requires `destination` ∈ font/script/style/worker, and a JSON `fetch` has `destination: ""`. So repeat visits pay an ETag revalidation round trip. (b) `src/proxy.ts:147`'s matcher excludes `_next/static`, `_next/image`, `favicon.ico` and image extensions but **not `.json`**, so the 2.53 MB payload traverses per-request nonce generation and CSP stamping. Ship the cache header and a narrowed matcher first; content-hashed filenames + `immutable` are a separate, larger item and unsafe until the filenames actually carry a hash.
...
(a) `getUser()` → `GET /api/account/preferences` → a conditional `PUT` bootstrap awaited inside the GET's `.then`. The PUT only seeds server-side defaults and **nothing reads a "has been bootstrapped" signal**, so the await buys nothing — fire-and-forget is safe.

(b) **Corrected 2026-07-29 — originally overstated as a 3-hop first-paint waterfall.** The 4-way fan-out is **not on first paint**: it is gated on drawer visibility (`ClinicalDashboard.tsx:1435-1461`), and the mount effect requests `includeDashboardData: false`. Mount runs 2 serial hops, and the first of them is dead weight in production — `readLocalProjectIdentity` resolves through `local-project-guard.ts:23-44`, where a non-localhost host makes `safeLocalOrigin` unconditionally true. It is load-bearing on localhost only. Real production win ≈ 1 hop.

(c) Per evidence image: mount → IntersectionObserver → signed-URL fetch → first image byte, i.e. 2 sequential round trips before any image data. **Corrected 2026-07-29 — the fix is mostly already built.** `src/app/api/images/signed-urls/route.ts` is a batch endpoint (POST `{imageIds}`, max 100, owner-scoped, generation-gated) with **zero call sites in `src/`**. `VisualEvidenceCard` already carries `image_id` and `signed_url_endpoint` (`evidence.ts:394`), and `setCachedSignedUrl` is exported, so a batch response can pre-seed the client LRU with no shape change. **Hard constraint:** never attach signed URLs to `SearchResult.images[].signed_url` — `rag-cache.ts:383-470` persists `results` into the `rag_response_cache` table, which would write bearer credentials into a shared cross-owner table and serve them dead. `buildVisualEvidence` output (`search/route.ts:838`) sits outside the cached object and is the only safe attachment point.

Worth noting the good news: **zero** route files are client components — all ~120 `src/app/**` pages are Server Components.
...
2. **Offline round-trip budget harness.** Wrap the Supabase client in a counting proxy; assert per-scenario query budgets against the existing offline surface — `scripts/eval-rag-offline.mjs`, `scripts/test-rag-offline.mjs`, `scripts/rag-offline-contract.mjs` and `scripts/fixtures/rag-offline-contract-tests.json` (**corrected 2026-07-29: an earlier draft named `test-cache-path.mjs`, which is the vitest/tsc cache-directory helper and unrelated**). Drive it the way `tests/rag-offline-answer.test.ts:79-108` already does — `vi.doMock("@/lib/supabase/admin", …)` before importing `rag.ts`, which is the only zero-production-edit seam because `rag.ts` and `rag-cache.ts` construct their client inline per phase. Ship it as a vitest suite rather than a new npm script, so it rides `npm run test` instead of forcing a `ci.yml` edit via `check-gate-manifest.mjs`. Converts "an extra uncached round trip" from inference into a pinned integer **and leaves a permanent regression guard**. Zero providers, zero DB.
