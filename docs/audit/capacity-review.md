...
Topology assumptions come from `docs/architecture/deployment-architecture.md` (single warm
...
connections** (advisor finding, recorded in `docs/guides/process-hardening.md`). Auth
...
1. **Switch auth to percentage-based connection allocation** in the Supabase
   ...
   `docs/operations/auth-connection-cap-runbook.md` (not settable via SQL/MCP — operator
...
latency SLOs by route mode exist (`docs/operations/observability-slos.md`).
...
1. **First hard failure: the auth 10-connection cap** during synchronized
   ...
   — exact operator path + verification in `docs/operations/auth-connection-cap-runbook.md`
