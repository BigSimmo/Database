...
- Buckets `clinical-documents` and `clinical-images` are **private**
  ([docs/architecture/multi-user-auth-setup.md](../architecture/multi-user-auth-setup.md) §7).
...
- **Evidence:** the live app and worker are recorded in Railway Singapore
  ([deployment-architecture.md](../architecture/deployment-architecture.md)); the OpenAI client uses
...
- **Progress (2026-07-13):** fixes (2)+(3) are **live on `main`** via PR #513
  ...
  in **[docs/security/openai-cross-border-basis.md](openai-cross-border-basis.md)**, which also records that
