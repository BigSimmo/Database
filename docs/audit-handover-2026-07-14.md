...
- **No confirmed P0/P1 defects in application source** that equal an active cross-tenant breach or
  clinical governance bypass. This scoping is deliberate: other P1-severity issues exist but fall
  outside those two defect classes.
...
| ID | Finding | Evidence | Remediation wave |
| --- | --- | --- | --- |
| C1 | `.env.example` weak-OR flag `true` vs code default `false` | env example vs `env.ts` | Wave F5 |
