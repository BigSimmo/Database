...
- ⚠️ **Ward Board is PAUSED BY THE OWNER** (_"Hold off for now"_). **It HOLDS `ward-screen.tsx` and
...
_(This heading said "the three standing traps" and listed four within the hour. A rule that COUNTS is
brittle; the fix is to stop carrying a count, not to correct it. Third time tonight.)_
