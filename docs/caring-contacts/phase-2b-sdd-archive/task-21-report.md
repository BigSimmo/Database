...
**F5 — a generated file changed under this task and is left uncommitted.**
`data/outstanding-issues-snapshot.json` was clean at session start and is now modified: `pending` 47 →
36, with eleven inbox request entries dropped. It is regenerated by `prebuild` → `snapshot:issues`.
Nothing in this task touched the issues inbox, so this is the committed snapshot being stale against
the inbox this branch actually holds, surfaced by a build. **Left uncommitted deliberately**; it is
not this task's to land.
...
run again after it was written, returning the same line. The browser gate above ran on the code
