...
| **Prove** | `⏸` row counts `rag_queries` not like `redacted-query:%` → expect `0`; lexical probe from process-hardening |
...
| **Owner** | OWNER:OPS |
| **Address** | Set/confirm same ≥16-char value in Railway runtime (prod + staging). Confirm boot smoke and deep readiness. |
...
| **Owner** | OWNER:OPS |
| **Address** | Restore embedding/completions quota → run `eval:retrieval:quality` (36/36) → `eval:quality -- --rag-only` → `npm run verify:release` per [`launch-operator-runbook.md`](launch-operator-runbook.md) §2. |
...
| **Owner** | OWNER:CODE |
| **Address** | Remove `aria-hidden` from description span in `mode-action-popup.tsx` (keep description out of the accessible *name*). |
...
| --- | --- | --- | --- |
| P3-R1 | Demotion floor `Math.max(hybrid_score, boosted)` | Clamp below hybrid only for explicit demotion delta; or subtract after floor; golden-eval gate any change | `eval:retrieval:quality` 36/36 |
...
4. `⏸ npm run eval:retrieval:quality` + `eval:quality -- --rag-only`  
