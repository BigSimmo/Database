...
| Dictionary catalogue (`/dictionary/search`)                  | In-flow shared composer at every width, under mode nav and above the Filter band; no phone bottom dock even with a query | Page content; idle phone reserve (not a submitted-results dock)                |
