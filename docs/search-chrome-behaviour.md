...
- **Feed the reporter from the element that actually scrolls.** `GlobalSearchShell`'s `#main-content` is the scrollport only on phones, so above that it also runs `useDocumentScrollHideReporter`. That hook self-gates: the fixed, overflow-clipped `.phone-viewport-shell` keeps the document from scrolling and never fires. Size it only with `inset: 0` — never `100vh`, `100svh`, or `100dvh` — so Safari tabs and newly launched standalone PWAs follow the live viewport without an unpainted bottom band.
