...
> ⚠️ **AND VERIFIER NAMED THE REAL COST, WHICH IS NOT THE WASTED ERRAND:** _"If the ledger says I
> assessed those surfaces, then my FIRST look at them will be received as an independent SECOND
> look."_ ✅ **A false attribution does not just fail to supply evidence — it manufactures
...
**I told it _"the owner has restarted you — his word, just now."_ It did not act on it.** **He had five
ward chats open and _"restart ward chat"_ names none of them** — **and, more to the point, he had
...
**⚠️ _"When in doubt, hide it"_ reads as safe, and on this page it has a CLINICAL COST.** **With Ward
...
| **Record the SUBURB, or the ARRIVAL TIME?**              | ✅ **BOTH** — _"Yes to both"_                                                                                       | Ward Core, the model             |
...
⚠️ **THE AMBIGUITY THAT NEARLY COST THE KEYSTONE, AND HOW IT ENDED:** _"2. Yes record"_ arrived
...
1. **A numbered list is an ADDRESS. Whoever is SECOND into a chat names the SUBJECT, never the
   number.** _(Three sessions broke this inside one exchange, each having just stated it.)_
...
⚠️ **THE COMPONENT'S OWN DOC COMMENT NAMES THIS EXACT CASE:** _"a median of one, rendered bare, is a
...
> _"a measure computed from a thin sample **must say so in the same breath as the figure**, not in a
> tooltip or a footnote — a median of one, **rendered bare**, is a guess wearing the clothes of a
> measurement, and this board's rule is to say nothing rather than guess."_

⚠️ _**"Say nothing rather than guess" attaches to "rendered BARE".**_ **The clause before it is the
...
_"Not enough data to compute"_ only when the value is genuinely `undefined`.**
...
**AND THE SUBSTANTIVE QUESTION IS STILL REAL AND STILL HIS:** _should a figure labelled **Median** be
printed at all from one observation, even with its basis beside it?_ ⚠️ **A fresh product judgement
...
label says _Median_, which a clinician reads as a typical case regardless of what sits next to it.**
...
is to be trusted about its own limits — and the number is labelled _Median_.** ⚠️ **A psychiatrist
reading _"30 min"_ reads a typical case.**

**RECOMMENDED, NOT DONE:** suppress below a stated minimum (`n < 5` → _"Not enough data to compute"_,
...
| 3   | **The referral screen** — _"the referrer chooses where to refer, and is shown why"_ | referrals — `521888a23`                      | ✅                         |
...
typechecking (`3c5e694cf`, and its message is the finding — _"which the test suite could never have
told me"_); and the transport-cancel permission fix (`64c434355`).**
...
· **`HD-Q1`: `/transport`, _"Live tracker"_ and `live-tracker.tsx` are three names for one page, and a
...
above a more urgent person.** His words: _"Ok I agree with that rule."_
...
**2. ⚠️ NEW: _"patients must met a certain high threshold to be marked as urgent."_**
...
> meet exactly here:**
>
> - _**"Nothing predicts, scores, ranks or recommends a person"**_ — **a numeric threshold the
>   software applies breaches this.**
> - _**"No figure, timeframe or threshold from the Mental Health Act, anywhere"**_ — **a sourced
...
because _"high threshold"_ SOUNDS numeric. That is the one shape both refusals forbid, and it will
...
> ward** — the ward page, the board's _"Coming in"_ panel, and the new hub. **The owner objected to
...
| 0.4 | ✅ **FIXED at `8ff7ec313`.** `DEMONSTRATION_DAY_LABEL` + `JURISDICTION_LABEL` in `ward-sites.ts`, wording changed to _"Scenario set on…"_                                                                                                                                                                     | no                 |
...
figures AND times can differ, pinned by a test on the word _"figures"_, so nothing on screen
...
| 0.4 | `DEMONSTRATION_DAY_LABEL` + `JURISDICTION_LABEL`, wording changed to _"Scenario set on…"_                                                                                         | ✅               |
...
actually said (_"nothing writes them out of order today; the shape permits it"_). **A type-level
...
queued as live defects.** ⚠️ **I wrote the rule _"re-measure before queueing"_ after finding the
...
was specific: **a screen asserted that a real body had ACTED** — _"St John WA accepted, awaiting
departure"_ — **when it has agreed to nothing.** **A site list naming the hospitals a WA bed-flow hub
...
1. ✅ **THE COMMUNITY HUB IS CONFIRMED — he settled it himself: "Build community hub. I give
   permission."** He had parked it directly this morning (_"I park community hub"_), and **both halves
...
_"Hold off for now."_ It stopped before touching anything. Its tree is clean at `09b4a9e87`.**
...
**No reason was given and none was asked for** — **following _"hold off"_ with a question about
...
⚠️ **READ THE CORRECTION CAREFULLY, BECAUSE THE OBVIOUS INFERENCE FROM IT IS WRONG:** _"Verifier
did not assess them, so Verifier may build them"_ — **that would trade the instrument for an extra
...
| ⚠️ **`TR-F2` — the reducer names TWO REAL ORGANISATIONS on screen**, chosen by nobody | Core        | _"St John WA accepted, awaiting departure"_. **Synthetic-data breach, live**                |
...
**The reload risk is the owner's to know about, not a task:** `D9-8` already lists _"the prototype's
memory"_ and it is **cut**. **The demonstration risk is not the same thing as the feature gap** — it is
...
- ✅ **THE COMMUNITY HUB IS UNPARKED — owner, 2026-08-30, later the same day.** He parked it directly
  (_"I park community hub"_), then asked for **"Community versions"** among the screens to build.
...
**This section was headed _"approved, not built"_ for a day after the last of them landed. Corrected
...
_"Just have 10 placeholder urgent reasons for now"_). **Eight of the ten describe what the current
...
> NAMED PERSON on a screen many services can see, is a DISCLOSURE** — and _"currently secluded or
> restrained"_ is a significant one.
...
> _"needs a level of observation this ward would have to provide"_ rather than _"currently secluded or
> restrained"_. **The same move the parallel badge makes, applied to facts instead of destinations.**
...
**Checked and NOT a finding:** `OVERRIDE_REASONS` reads as clinical in places — _"Clinical urgency
outweighs the mismatch"_, _"Closer to the person's home or family"_ — **but they are THE OWNER'S OWN
...
> must work out who books.** ⚠️ **_"Always"_ is load-bearing: a future case that feels different needs
...
> **`Q1` — the two-clinician review — CLOSED by him:** _"close this two clinician review for now and
> mark as closed."_ ⚠️ **Recorded as a decision to STOP SEEKING the evidence, not the arrival of it.
...
clinician's behalf — and the line directly beneath it says so about the legal form:** _"The clinician
picks one; the software never picks one for them."_
...
change-pickers, and had _"there is no raise-referral urgency picker in this file"_ half-written
