...
Curated map of `scripts/` (227 files) and the `package.json` script surface (239 entries),
...
| `check-outstanding-issues.mjs`, `check-ledger-write-discipline.mjs`, `check-pr-mergeability-workflow.mjs`                                                                                                                                                               | Outstanding-issues ID/marker/no-driver guard; immutable-record and serial-reconciliation transaction guard; PR mergeability workflow contract                                                                                                                                                                                                                                                                                                                                                                 |
| `outstanding-issues.mjs`, `ledger-inbox.mjs`                                                                                                                                                                                                                            | Canonical outstanding-issues writer plus conflict-free PR inbox. `issues:add` / `issues:done` / `issues:update` add one immutable request file; one dedicated `issues:reconcile` pass applies them to `docs/outstanding-issues.md`, allocates IDs, and preserves the applied request trail. Never hand-edit the canonical file.                                                                                                                                                                               |
...
- `branch-review-ledger.mjs` — the **only** way to read the historical review corpus or create a
  new immutable review record (`ledger:lookup` / `ledger:append`). `ledger:migrate-legacy` is the
  one-time active-branch conversion path for a pre-system shared-table row. Do not hand-write a row.
- `check-ledger-write-discipline.mjs` — blocks a dated-row change in the frozen historical table
  and proves canonical issue-ledger edits against their moved immutable requests.
...
- `list-database-skills.mjs` (`skills` / `check:skills`), `sync-skills.mjs`, `skill-create.mjs` —
  render the `.agents/skills/` catalogue and validate policy across all repository skill surfaces.
