...
Curated map of `scripts/` (204 files) and the `package.json` script surface (215 entries),
...
| `setup-codex-cloud.sh`, `maintain-codex-cloud.sh`, `install-codex-cloud-command-shims.sh`, `delete-codex-cloud-branch-with-pat.sh`, `check-codex-cloud-setup.mjs`, `ensure-codex-cloud-git-remote.mjs` | Reproducible Codex Cloud toolchain/profile setup, profile-loading Node command shims, a connected-only guarded PAT branch-deletion exception, sanitized acceptance, and safe credential-free `origin` repair                                  |
