...
- `MAX_UPLOAD_MB` and `NEXT_PUBLIC_MAX_UPLOAD_MB` are both declared as Docker
  build args before `npm run build`, so the parity guard sees the same lowered
  non-secret limit that the browser build will inline. Mirror the same value in
  both Railway service variables before rebuilding; Railway still injects the
  authoritative server value at runtime, while Next freezes the public mirror
