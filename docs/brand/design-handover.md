...
| `psychsift-mark-light.svg`                | The mark on a pale tile, for light grounds                                                                                       |
...
Every curve of the mark is a circular arc. The glyph is drawn in a 100-unit design space and its ink
box measures 55.331 × 100.3813; that box is what every centring transform below is derived from.
...
| Tail hook    | 55.1234 | (51.5936, 16.1417)                             |
...
furthest ink 160 px out against the 171 px allowed. Do not raise that 62%.
...
- Cut opens from 4.2000 to 7.2000: the two facing edges move 3.0 units apart, 1.4493 on the upper
  edge and 1.5507 on the lower, since the point reflection fixes how the 3.0 splits. Only the facing
  edges move, so the outer silhouette is identical to the primary file's.
...
- **Wordmark** — 20 px, weight 800, tracking −0.02 em (`--tracking-display`), `--text-heading`
...
| `scripts/generate-brand-assets.ts`                           | Regenerates **two** targets: `src/app/icon.svg` and the mark inlined in `public/offline.html`. Changing the offline page also means bumping `CACHE_VERSION` in `public/sw.js` and the hash pinned in `tests/pwa-manifest.test.ts` |
...
| `src/app/ckb-v2-tokens.css`                                  | The **effective** token layer — every surface and text swatch above resolves here, not in `globals.css`                                                                                                                           |
| `src/app/globals.css`                                        | The base layer: the tracking ladder, and the accent tokens the v2 layer does not override                                                                                                                                         |
