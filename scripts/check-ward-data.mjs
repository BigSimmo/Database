...
export function checkBedArithmetic({ sites, unitCapacity }) {
  ...
  for (const site of sites) {
    for (const unit of site.units ?? []) {
      ...
      if (![beds, empty, allocatable, blocked].every((value) => Number.isInteger(value) && value >= 0)) {
        continue;
      }
  ...
  for (const [region, bySite] of Object.entries(travelBands ?? {})) {
    ...
    for (const [code, band] of Object.entries(bySite ?? {})) {
      if (!knownCodes.has(code)) {
        ...
        problems.push(
          problem("travel-times-name-an-unknown-hospital", `Travel times: hospital code "${code}" (under ${region})`, [
            `${FILES.travel} records a travel time to a hospital with the code "${code}", but no hospital in ${FILES.sites} has that code.`,
            suggestion ? `Did you mean "${suggestion}"?` : `The hospital codes are: ${listOut(siteCodes)}.`,
      ...
      if (!knownBands.has(band)) {
        problems.push(
          problem("travel-times-use-an-unknown-band", `Travel times: ${region} to ${code}`, [
...
export function formatReport(result) {
  ...
  return [
    `${plural(problems.length, "problem")} found in the Ward Flow data.`,
    ``,
    ...problems.map((entry) => `${entry.text}\n`),
    summary,
  ].join("\n");
}
...
function refuseAnEmptyRun(data) {
  ...
  if (!Array.isArray(data.admissions) || data.admissions.length === 0)
    missing.push(`the people in the beds (${FILES.seed})`);
  if (!Array.isArray(data.homeRegions) || data.homeRegions.length === 0)
    missing.push(`the list of regions (${FILES.model})`);
  if (!Array.isArray(data.serviceOrder) || data.serviceOrder.length === 0)
    missing.push(`the health-service order (${FILES.derivations})`);
