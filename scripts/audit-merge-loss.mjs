 ...
 * the window, compare the ref's current blob for every file that landing
 * changed against that file's blob at the landing's first parent. Equality
 * means the landing's contribution to that file is no longer present. Blob OIDs
 * are compared rather than content, so this stays cheap over a wide window.
 *
 ...
 * when the identically-named audit record actually exists at the target ref.
 */
export function isReconciliationMove(file, ref, blobAt) {
  const match = new RegExp(`^${INBOX}/([^/]+\\.json)$`).exec(file);
  if (!match) return false;
  return blobAt(ref, `${INBOX}/applied/${match[1]}`) !== null;
}

/**
 * Compare each landing's contribution against the ref's current state.
 *
 * `blobAt(ref, file)` returns a blob OID, or null when the path does not exist
 * at that ref. Injected so the whole classifier is testable without git.
 *
 ...
 * @param {{ landings?: Landing[], blobAt: (ref: string, file: string) => string | null,
 *   ref?: string }} options
 */
export function classifyMergeLoss({ landings = [], blobAt, ref = DEFAULT_REF }) {
  const findings = [];
  let filesCompared = 0;
  let filesExempted = 0;
  const skipped = [];
  for (const landing of landings) {
    ...
    for (const file of landing.files ?? []) {
      filesCompared += 1;
      const before = blobAt(landing.preRef, file);
      const now = blobAt(ref, file);
      if (before !== now) continue;
      if (now === null && isReconciliationMove(file, ref, blobAt)) {
        filesExempted += 1;
        continue;
      }
...
function blobReader() {
  ...
  return (ref, file) => {
    const key = `${ref}:${file}`;
    if (!cache.has(key)) cache.set(key, tryGit(["rev-parse", `${ref}:${file}`]) ?? null);
...
function report(result, { ref, since, strict }) {
  ...
  for (const finding of findings) {
    ...
    console.log(
      `    ${finding.revertedFiles.length} of ${finding.changedFiles} changed file(s) match the pre-merge blob:`,
    );
...
function selfTest() {
  ...
  const blobAt = (ref, file) => blobs.get(`${ref}:${file}`) ?? null;
  ...
  const reconciled = classifyMergeLoss({
    ref: "head",
    blobAt: (reference, file) =>
      reference === "head" && file === `${INBOX}/applied/${path.posix.basename(request)}` ? "eee" : null,
  ...
  console.log("merge-loss audit self-test passed.");
}
...
function main() {
  ...
  const result = classifyMergeLoss({ landings, blobAt: blobReader(), ref: options.ref });
  if (options.json) {
