...
const SCOPED_ALLOWLIST = new Map([
  ...
  // ─── Ward Flow, added 2026-09-03 with the publication branch ────────────────────────────────
  // Every entry below is a path a Ward Flow document names ON PURPOSE and that does not exist:
  // a scratch probe created to produce evidence and then removed, a file proposed but not built,
  // a citation the document itself is reporting as WRONG, or a path that moved and whose
  // non-resolution is the finding. Correcting any of them would destroy the record it carries.
  [
    "docs/ward-flow/cleanup-awaiting-approval.md",
    // The two untracked scratch probes this document exists to ask the owner about deleting.
    // The table names them so somebody can find them in the worktree they block.
  ...
  [
    ...
    // The 118.5 MB disaster-recovery bundle directory, deliberately absent from this published
    // branch: GitHub refuses any push carrying a blob over 100 MB. The receipts state the
    // requirement, which is what a later restore needs to read.
    new Set(["docs/ward-flow/control/evidence/bundles/"]),
  ],
  [
    "docs/ward-flow/register/ward-builder-one-findings.md",
    // Finding WB1-013 IS that a source comment cites this path and the real file is
    // `tests/ward-morning-tour-paused.dom.test.tsx`. The wrong path is the subject.
    new Set(["tests/ward-morning-tour-paused.test.ts"]),
  ],
  [
    "docs/ward-flow/register/ward-builder-two-findings.md",
    // Row WB2-101 records that it cited this file until 2026-09-03 and that the file has never
    // existed in the branch's history. The mutation was real; the citation was not.
    new Set(["tests/ward-flow-module-graph.test.ts"]),
  ],
  [
    "docs/ward-flow/rescued/ward-builder-three/wf-build3-006-triage-progress.md",
    // An elided path written for the reader's eye in a narrow table column; the same document
    // gives the full path in its opening line.
    new Set(["docs/…/wf-build3-005-ts-test-sweep.md"]),
  ],
  [
    "docs/ward-flow/sdd-rescued/builder-plan-2026-09-01/task-statistics-report.md",
    // An untracked scratch probe the report names so the next session can remove it, quoting
    // the exact command. Naming it is the point.
    new Set(["tests/zz-scratch-probe.test.ts"]),
  ],
  [
    "docs/ward-flow/triage/reexport-blindness-sweep.md",
    // Not a path at all: a quoted import-matching regex whose `["']([^"']+)` fragment parses as
    // a markdown link with `[^"']+` as its target. The regex is evidence and is quoted verbatim.
    new Set(["[^\"']+", "docs/ward-flow/triage/[^\"']+"]),
  ],
  [
    "docs/ward-flow/triage/wf-build2-006-batch-b.md",
    // A proposed new module ("**New file**") that was never built, and a line-range citation
    // into a file that does exist — the `:1579–1598` suffix is what fails to resolve.
    new Set([
      "src/components/ward-management/ward-release-notes.ts",
      "tests/ward-legal-figure-guard.test.ts:1579–1598",
    ]),
  ],
  [
    "docs/ward-flow/triage/wf-build2-006-batch-c.md",
    // Another proposed-and-not-built module, named as the subject of a dynamic-import finding.
    new Set(["src/components/ward-management/ward/ward-referral-dynamic-leak.ts"]),
  ],
  [
    "docs/ward-flow/wf-build3-005-ts-test-sweep.md",
    // The document's own FALSIFIER: "re-create the page at ..." — a path that must not exist for
    // the finding to hold.
    new Set(["src/app/(search-app)/ward-management/page.tsx"]),
  ],
  [
    "docs/ward-flow-ledger.md",
    // A decision document named in the WB-DB-18 row and not written; the ledger row carries the
    // decision itself, which is the durable part.
    new Set(["docs/ward-flow-decision-wb-db-18.md"]),
  ],
  [
    "docs/ward-flow-orchestrator-handover.md",
    // "the structure moved" — this glob NOT resolving is the evidence the sentence offers.
    new Set(["src/app/ward-management/**"]),
  ],
  [
    "docs/ward-flow-safety-checklist.md",
    // Same glob, same argument, stated even more explicitly: "`src/app/ward-management/**` no
    // longer resolving proves THE LAYOUT MOVED." Plus a control script that lives on
    // `claude/Wardquestions`; the document gives the `git show` command to retrieve it.
