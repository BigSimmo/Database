...
if (!checkOnly) {
  ...
  // The generation being replaced stays on disk for one more deploy. A client
  // loaded before this deploy has the *previous* hashed URL compiled into its
  // bundle, and pruning it immediately would 404 that session the moment it
  // opened a catalogue route — the deployment-straddling failure the
  // unversioned aliases were added to prevent, reintroduced one level down.
