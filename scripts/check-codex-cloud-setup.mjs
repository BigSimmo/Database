...
export function validateCodexCloudSetup() {
  ...
  requireMatch(errors, commandShims, /nvm which/, "Cloud command shims must resolve the selected Node version through nvm.");
  requireMatch(errors, commandShims, /\.clinical-kb-codex-cloud\.sh/, "Cloud command shims must load the generated profile.");
  if (!commandShims.includes('exec "$node_bin/$command_name" "\\$@"')) {
    errors.push("Cloud command shims must execute absolute Node commands.");
  }
  requireMatch(errors, patDelete, /CODEX_CLOUD_ACCESS_PROFILE.*connected/, "PAT deletion helper must require the connected profile.");
  ...
  requireMatch(errors, patDelete, /https:\/\/github\.com\/BigSimmo\/Database\.git/, "PAT deletion helper must require the credential-free origin.");
  requireMatch(errors, guide, /bash scripts\/setup-codex-cloud\.sh/, "The guide must provide the setup command.");
  requireMatch(errors, guide, /install-codex-cloud-command-shims\.sh/, "The guide must document the command-shim workaround.");
