-- Index-friendly lexical retrieval (2026-07-13 audit, finding 1).
--
-- The previous body matched with a single predicate that OR-ed conditions
-- across two relations:
--   (c.search_tsv @@ query.tsq or d.title_search_tsv @@ query.tsq)
-- PostgreSQL cannot serve that disjunction from either GIN index, so every
-- call sequential-scanned the full document_chunks table (~17s warm at 70k
-- chunks in production, ~5.5 GB of buffer traffic per call, up to three
-- lexical variants concurrently). Probing document_chunks_search_idx and
-- documents_title_search_idx separately and unioning the candidate chunk ids
-- returns the same rows from two index probes; the visibility gates then run
-- on the narrowed candidate set only.
--
-- Contract is unchanged: same signature, same 18 output columns, same score
-- formulas (text_rank / similarity 0 / capped hybrid_score / lexical_score),
-- same candidate cap and ordering, service-role-only execution. Rows tied on
-- text_rank at the candidate-cap boundary were already planner-order
-- dependent; that is unchanged. Parity + plan harness:
-- scripts/sql/lexical-rpc-parity-check.sql.
