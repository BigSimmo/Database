-- Forward-codify the live bodies of three hybrid retrieval RPCs that the migration
-- chain never received: match_document_embedding_fields_hybrid,
-- match_document_index_units_hybrid, match_document_memory_cards_hybrid_v2.
--
-- Plan of record: docs/database-remediation-plan.md Phase 3 ("live-ahead: codify the
-- live body into a new migration + schema.sql mirror"); evidence:
-- docs/audit/live-drift-forensics-2026-08.md section Phase 3 (3.5, staging proof).
-- Ledger anchor #316.
--
-- WHY: 20260712000000_forward_codify_retrieval_owner_matches.sql documented that all
-- eight primary retrieval RPCs on live gate ownership through the fail-closed,
-- sentinel-aware helper public.retrieval_owner_matches(owner_filter, d.owner_id),
-- and left the byte-perfect body codification to an owner step. Later migrations
-- (20260712171500, 20260714110000, 20260724120000) codified five of them, but these
-- three were never re-created from live: their newest committed body is still
-- 20260701140631, which carries the legacy inline predicate
-- `(owner_filter is null or d.owner_id = owner_filter)` (fail-open on NULL,
-- sentinel-blind). Measured on staging after the byte-verified chain replay plus
-- 20260818110000: def_hash still differs from the manifest for exactly these three,
-- and pg_get_functiondef shows the legacy predicate where schema.sql has
-- retrieval_owner_matches. This is NOT a production tenancy hole — production ==
-- schema.sql == drift manifest for all three (def_hash equal, forensics 3.1) — it is
-- a reproducibility hole for any environment built from migrations alone (staging,
-- preview branches, `db reset`, DR restore, fresh eval databases).
--
-- Definitions below are verbatim from supabase/schema.sql (the manifest source),
-- including every SET clause, so CREATE OR REPLACE preserves the live proconfig
-- [search_path, (plan_cache_mode), work_mem]; ACLs are untouched by CREATE OR
-- REPLACE. Idempotent on production (sjrfecxgysukkwxsowpy): identical text, no-op.
-- RAG impact: no retrieval behaviour change — codifying already-live bodies.
