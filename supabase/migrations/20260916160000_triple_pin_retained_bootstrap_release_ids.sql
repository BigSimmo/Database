-- Triple-pin retained-bootstrap CHECK constraints and
-- read_site_content_public_records bootstrap predicates for the forms
-- handover re-key (ddc94ecf) while live / prior schema still carry e4a1dd29
-- and 91ceaa8d.
--
-- WHY. P03 refresh on #2821 re-keyed the epoch-zero population onto
-- ddc94ecf-3527-5b4d-846b-af5724b428ca across schema.sql and the historical
-- bootstrap migration text. An applied migration is not re-run, so live still
-- carries the e4a1dd29 pins (and some environments may still hold 91ceaa8d from
-- the #2814 re-key) in:
...
-- while a freshly replayed schema seeds ddc94ecf. The reader migration
-- 20260916103000 still hard-codes only e4a1dd29, so a fresh replay that seeds
-- ddc94ecf then marks that release invalid and returns no bootstrap records.
-- See src/lib/site-content/site-content-health.ts (RETAINED_BOOTSTRAP_RELEASE_IDS).
--
...
-- bootstrap identities, matching the health probe. Existing live rows with
-- e4a1dd29 / 91ceaa8d continue to satisfy; a freshly replayed schema that seeds
-- ddc94ecf also satisfies. No data rewrite.
--
...
-- merged, the integration records version 20260916160000 as applied, so deleting
