...
--
-- Valid redacted token formats:
-- - redacted-query:<64-char-hex> (HMAC-SHA256 from src/lib/query-privacy.ts)
-- - redacted-query:legacy:<32-char-hex> (this migration's salted placeholders,
--   so re-running the migration is a no-op on already-scrubbed rows)
-- - redacted-cache:<64-char-hex>
-- Only values matching a complete token shape are preserved; prefix-only
-- matches that don't match the full format are treated as legacy plaintext
-- and scrubbed.

update public.rag_queries
set query = 'redacted-query:legacy:' || md5(gen_random_uuid()::text || query)
where query !~ '^redacted-query:([0-9a-f]{64}|legacy:[0-9a-f]{32})$';

update public.rag_query_misses
set
  query = case
    when query ~ '^redacted-query:([0-9a-f]{64}|legacy:[0-9a-f]{32})$' then query
    else 'redacted-query:legacy:' || md5(gen_random_uuid()::text || query)
  end,
  normalized_query = case
    when normalized_query ~ '^redacted-query:([0-9a-f]{64}|legacy:[0-9a-f]{32})$' then normalized_query
    else 'redacted-query:legacy:' || md5(gen_random_uuid()::text || normalized_query)
  end
where query !~ '^redacted-query:([0-9a-f]{64}|legacy:[0-9a-f]{32})$'
   or normalized_query !~ '^redacted-query:([0-9a-f]{64}|legacy:[0-9a-f]{32})$';
...
set
  ...
  normalized_query = case
    when normalized_query is null or normalized_query ~ '^redacted-query:([0-9a-f]{64}|legacy:[0-9a-f]{32})$' then normalized_query
    else 'redacted-query:legacy:' || md5(gen_random_uuid()::text || normalized_query)
  end
where query !~ '^redacted-query:([0-9a-f]{64}|legacy:[0-9a-f]{32})$'
   or (normalized_query is not null and normalized_query !~ '^redacted-query:([0-9a-f]{64}|legacy:[0-9a-f]{32})$');
