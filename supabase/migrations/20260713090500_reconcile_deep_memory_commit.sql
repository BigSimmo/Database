-- Forward-apply the canonical deep-memory commit body after the original
-- production migration version was recorded with an earlier implementation.
-- This changes only the function definition and its service-role-only ACL.
