...
begin
  if not exists (
    ...
    join pg_catalog.pg_class t on t.oid = c.conrelid
    join pg_catalog.pg_namespace n on n.oid = t.relnamespace
    where n.nspname = 'caring_contacts' and t.relname = 'contacts' and c.conname = 'contacts_team_plan_fk'
  ) then
...
begin
  if not exists (
    ...
    join pg_catalog.pg_class t on t.oid = c.conrelid
    join pg_catalog.pg_namespace n on n.oid = t.relnamespace
    where n.nspname = 'caring_contacts' and t.relname = 'contact_dispatches' and c.conname = 'contact_dispatches_team_contact_fk'
  ) then
...
begin
  if not exists (
    ...
    join pg_catalog.pg_class t on t.oid = c.conrelid
    join pg_catalog.pg_namespace n on n.oid = t.relnamespace
    where n.nspname = 'caring_contacts' and t.relname = 'retention_state' and c.conname = 'retention_state_team_plan_fk'
  ) then
...
begin
  if not exists (
    ...
    join pg_catalog.pg_class t on t.oid = c.conrelid
    join pg_catalog.pg_namespace n on n.oid = t.relnamespace
    where n.nspname = 'caring_contacts' and t.relname = 'cultural_identity_reports' and c.conname = 'cultural_identity_reports_team_plan_fk'
  ) then
